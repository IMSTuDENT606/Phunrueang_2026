...
หากยังเห็น `permission_denied` ให้ตรวจสอบว่ากด Publish Rules แล้ว และบัญชีที่ใช้หน้าแอดมินมีอยู่จริงใน Firebase Authentication. Console จะมี log สำหรับ `initialized`, `Realtime listener attached`, `snapshot received`, `auth state` และ error code เพื่อช่วยตรวจสอบปัญหาบนอุปกรณ์ iPhone/iPad ด้วย
