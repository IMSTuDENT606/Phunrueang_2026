...
const pages = ['home', 'members', 'room-login', 'classroom-113', 'classroom-13', 'classroom-21', 'classroom-25', 'classroom-36', 'classroom-38', 'classroom-42', 'classroom-43', 'classroom-511', 'classroom-512', 'classroom-64', 'classroom-65', 'election', 'shop', 'admin', 'gallery', 'sports', 'tokens'];
...
function validateImageFile(file) {
  console.info('[Image] selected', { name:file?.name, type:file?.type, size:file?.size });
  if (!(file instanceof Blob) || !ALLOWED_IMAGE_TYPES.has(String(file.type || '').toLowerCase())) throw Object.assign(new Error('รองรับเฉพาะไฟล์ JPG, PNG และ WebP'), { code:'image/type-not-allowed' });
  if (file.size > MAX_IMAGE_UPLOAD_BYTES) throw Object.assign(new Error('ไฟล์รูปต้องมีขนาดไม่เกิน 12 MB'), { code:'image/file-too-large' });
}
// Temporary Data URLs are used only inside the editor/crop flow. They must be
// replaced by a Cloudinary URL before shared data is persisted.
async function prepareImageForStorage(file, options = {}) {
  validateImageFile(file);
  console.info('[Image] compression started', { size:file.size, maxSide:options.maxSide });
  const maxSide = Number(options.maxSide) || 1280, maxBytes = Number(options.maxBytes) || 320000;
  const source = URL.createObjectURL(file);
  try {
    const image = await new Promise((resolve, reject) => { const element = new Image(); element.onload = () => resolve(element); element.onerror = () => reject(new Error('ไม่สามารถอ่านไฟล์รูปภาพได้')); element.src = source; });
    let scale = Math.min(1, maxSide / Math.max(image.naturalWidth, image.naturalHeight)), quality = .82, result = '';
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      if (bytes <= maxBytes) { console.info('[Image] compressed', { bytes, width:canvas.width, height:canvas.height, type:'image/jpeg' }); return result; }
      if (quality > .58) quality -= .08; else scale *= .82;
    }
    console.info('[Image] compressed', { bytes:Math.ceil(result.length * .75), type:'image/jpeg' });
    return result;
...
function adminSaveErrorMessage(error) {
  ...
  if (/cloudinary\/config/i.test(code)) return 'ยังไม่ได้ตั้งค่า Cloudinary cloudName และ unsigned upload preset';
  if (/cloudinary\/upload/i.test(code)) return 'อัปโหลดรูปไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตและ Cloudinary upload preset';
  if (code === 'auth/admin-required' || /permission/i.test(code)) return 'สิทธิ์ผู้ดูแลหมดอายุ กรุณาเข้าสู่ระบบใหม่';
  if (code === 'firebase/not-ready' || /network|disconnect/i.test(`${code} ${error?.message || ''}`)) return 'เชื่อมต่อ Firebase ไม่ได้ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่';
  return 'บันทึกไม่สำเร็จ กรุณาลองใหม่ หรือตรวจสอบ Firebase Database Rules';
}

async function shrinkLegacyImage(value, options) {
  if (typeof value !== 'string' || !value.startsWith('data:image/') || value.length < 450000) return value;
  const blob = await fetch(value).then((response) => response.blob());
  return prepareImageForStorage(blob, options);
}
...
const FIREBASE_SHARED_KEYS = new Set([
  ...
  'phanuang-activity-attendance', 'phanuang-token-league-v1'
]);
...
let firebaseElectionVoteRoot = null;
let firebaseElectionVoteSnapshotReady = false;
let firebaseElectionVoteRecords = [];
...
const electionPerformanceNow = () => Math.round(performance.now() * 100) / 100;
const flattenElectionVotes = (value) => Object.values(value || {}).flatMap((election) => Object.values(election || {})).filter((vote) => vote && typeof vote === 'object');
const normalizeElectionVotes = (votes) => {
  const byStudent = new Map();
  votes.forEach((vote) => {
    if (!vote?.electionId || !vote?.studentId) return;
    const key = `${vote.electionId}/${vote.studentId}`;
    if (!byStudent.has(key) || Number(vote.time || 0) >= Number(byStudent.get(key).time || 0)) byStudent.set(key, vote);
  });
...
async function uploadMediaImage(value, folder = 'misc', onStatus) {
  ...
  if (!config.cloudName || !config.uploadPreset || Object.values(config).some((item) => String(item).startsWith('PASTE_'))) throw Object.assign(new Error('Cloudinary client config is incomplete'), { code:'cloudinary/config' });
  const blob = await fetch(value).then((response) => response.blob());
  const body = new FormData();
  body.append('file', blob, `image-${Date.now()}.${blob.type === 'image/png' ? 'png' : 'jpg'}`);
  body.append('upload_preset', config.uploadPreset);
  body.append('folder', `phunrueang/${String(folder).replace(/^\/+|\/+$/g, '')}`);
  onStatus?.('กำลังอัปโหลด');
  console.info('[Cloudinary] upload started', { folder, bytes:blob.size });
  try {
    ...
    if (!response.ok || !result.secure_url) throw Object.assign(new Error(result.error?.message || `Cloudinary HTTP ${response.status}`), { code:'cloudinary/upload-failed', status:response.status });
    console.info('[Cloudinary] upload success', { publicId:result.public_id });
    console.info('[Cloudinary] secure URL received', result.secure_url);
    onStatus?.('อัปโหลดสำเร็จ');
    return result.secure_url;
  } catch (error) {
    console.error('[Cloudinary] upload failed', { code:error?.code, message:error?.message, status:error?.status });
    onStatus?.('อัปโหลดไม่สำเร็จ');
    throw error;
  }
}
async function uploadEmbeddedImages(items, folder, onProgress) {
  const output = [];
  for (let index = 0; index < items.length; index += 1) {
    onProgress?.(index + 1, items.length, isEmbeddedImage(items[index]) ? 'กำลังบีบอัดรูป' : 'พร้อมใช้งาน');
    output.push(await uploadMediaImage(items[index], folder, status => onProgress?.(index + 1, items.length, status)));
  }
  return output;
}
...
function refreshRemoteViews() {
  ...
  if (active === 'home') { renderHomeStandMap(); renderHomeTeamDirectory(); }
  ...
  if (active === 'tokens') renderTokenLeague();
  if (active === 'admin') {
    ...
    } else if (activeTab === 'activity-attendance') {
      // Another admin may save while this browser is ticking names. Keep the
      // unsaved DOM intact; localStorage already contains the new cloud state
      // and will be used after this draft is submitted or the panel is reopened.
      if (!activityAttendanceDraftDirty && !document.querySelector('.activity-student-picker[open]')) {
        renderActivityAttendance($('#adminPanel'));
      }
    } else if(activeTab==='tokens') renderTokenAdminV3($('#adminPanel'));
    else renderAdmin();
  }
}
...
async function startFirebaseSync() {
  ...
  try {
    ...
    firebaseElectionVoteRoot = database.ref(`sites/${siteId}/electionVotes`);
    ...
    firebaseElectionVoteRoot.on('value', (snapshot) => {
      firebaseElectionVoteRecords = normalizeElectionVotes(flattenElectionVotes(snapshot.val()));
      firebaseElectionVoteSnapshotReady = true;
      console.info('[Admin] snapshot received', { performanceNow: electionPerformanceNow(), voteCount: firebaseElectionVoteRecords.length, path: firebaseElectionVoteRoot.toString() });
      const activeTab = document.querySelector('.admin-tab.active')?.dataset.adminTab;
      if (document.querySelector('.page.active')?.id === 'admin' && activeTab === 'election') renderElectionAdmin($('#adminPanel'));
    }, (error) => console.error('[Admin] election snapshot error', error?.code, error?.message));
...
const CLASSROOM_DATABASE = { '1/3': classroom13, '1/13': classroom113, '2/1': classroom21, '2/5': classroom25, '3/6': classroom36, '3/8': classroom38, '4/2': classroom42, '4/3': classroom43, '5/11': classroom511, '5/12': classroom512, '6/4': classroom64, '6/5': classroom65 };
...
function showPage(id) {
  ...
  document.body.classList.toggle('token-mode', id === 'tokens');
  ...
  const footerContext = {
    ...
    tokens: 'TOKEN LEAGUE · GRANDSTAND HEROES'
  };
...
async function submitVote() {
  ...
  const clickAt = electionPerformanceNow();
  const button = $('#submitVote');
  console.info('[Vote] click', { performanceNow: clickAt, electionId: config.electionId, studentId: student.studentId });
  if (!firebaseElectionVoteRoot || !firebaseElectionVoteSnapshotReady) { $('#voteError').textContent = 'กำลังเชื่อมต่อระบบลงคะแนน กรุณารอสักครู่แล้วลองใหม่'; return; }
  button.disabled = true;
  $('#voteError').textContent = 'กำลังบันทึกคะแนน…';
  const vote = { electionId: config.electionId, studentId: student.studentId, room: `${student.grade}/${student.classroom}`, candidate: String(selectedCandidate), time: Date.now() };
  const voteRef = firebaseElectionVoteRoot.child(config.electionId).child(student.studentId);
  try {
    console.info('[Vote] write start', { performanceNow: electionPerformanceNow(), elapsedMs: electionPerformanceNow() - clickAt, path: voteRef.toString() });
    const result = await voteRef.transaction((current) => current || vote, undefined, false);
    if (!result.committed) {
      $('#voteError').textContent = 'เลขประจำตัวนี้ใช้สิทธิ์เลือกตั้งแล้ว';
      return;
    }
    console.info('[Vote] write success', { performanceNow: electionPerformanceNow(), elapsedMs: electionPerformanceNow() - clickAt, electionId: config.electionId, studentId: student.studentId });
    const storedVote = result.snapshot.val() || vote;
    firebaseElectionVoteRecords = normalizeElectionVotes([...firebaseElectionVoteRecords, storedVote]);
    sessionStorage.setItem('phanuang-vote', JSON.stringify({ electionId: config.electionId, studentId: student.studentId, candidate: selectedCandidate, time: vote.time }));
    renderBallotCasting(config, selectedCandidate);
  } catch (error) {
    console.error('[Vote] write failed', { performanceNow: electionPerformanceNow(), elapsedMs: electionPerformanceNow() - clickAt, code: error?.code, message: error?.message });
    $('#voteError').textContent = error?.code === 'PERMISSION_DENIED' ? 'ระบบยังไม่อนุญาตให้เขียนคะแนน กรุณาตรวจสอบ Firebase Rules' : 'บันทึกคะแนนไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่';
  } finally {
...
function getVotes() {
  ...
  const allVotes = firebaseElectionVoteSnapshotReady ? normalizeElectionVotes([...legacyVotes, ...firebaseElectionVoteRecords]) : legacyVotes;
...
async function writeStoreMedia(productId,media){const key=`phanuang-store-media-${productId}`,serialized=JSON.stringify(media);await saveFirebaseSharedValue(key,serialized);console.info('[Firebase] image URL saved',{key});localStorage.setItem(key,serialized);const database=await openStoreMediaDatabase();return new Promise((resolve,reject)=>{const transaction=database.transaction(STORE_MEDIA_TABLE,'readwrite');transaction.objectStore(STORE_MEDIA_TABLE).put(media,productId);transaction.oncomplete=()=>{database.close();resolve();};transaction.onerror=()=>reject(transaction.error);});}
...
async function saveStoreLookbook(config){const image=await uploadMediaImage(config.image||'','products',status=>{$('#storeSaveState').textContent=status;});const metadata=JSON.stringify({sourceProductId:config.sourceProductId||'',finishProductId:config.finishProductId||config.sourceProductId||'',tolerance:Number(config.tolerance)||42});await writeStoreMedia('__lookbook_model__',{image});await saveFirebaseSharedValue(STORE_LOOKBOOK_KEY,metadata);localStorage.setItem(STORE_LOOKBOOK_KEY,metadata);config.image=image;}
...
const ORDER_ROOM_DATABASE = {
  '1/3': classroom13.students.map(([number,studentId,name])=>({number:String(number).padStart(2,'0'),studentId,name})),
  '1/13': classroom113.students.map(([number,studentId,name])=>({number:String(number).padStart(2,'0'),studentId,name})),
...
async function migrateProductImages(product,onStatus){const nextImage=await uploadMediaImage(product.image||'','products',onStatus),nextLookImage=await uploadMediaImage(product.lookImage||'','products',onStatus),nextLookImages=[];for(const image of product.lookImages||[])nextLookImages.push(await uploadMediaImage(image,'products',onStatus));product.image=nextImage;product.lookImage=nextLookImage;product.lookImages=nextLookImages;}
async function saveStoreProducts(products){for(const product of products){await migrateProductImages(product,status=>{$('#storeSaveState').textContent=status;});await writeStoreMedia(product.id,{image:product.image||'',video:product.video||'',lookImage:product.lookImage||'',lookImages:product.lookImages||[]});}const metadata=products.map(({image,video,lookImage,lookImages,...product})=>product),serialized=JSON.stringify(metadata);await saveFirebaseSharedValue(STORE_KEY,serialized);localStorage.setItem(STORE_KEY,serialized);console.info('[Firebase] image URL saved',{key:STORE_KEY});storeProductsRuntime=products;if(navigator.storage?.persist)try{await navigator.storage.persist();}catch(_){} }
...
async function renderStoreAdmin(host){
  ...
  const bind=()=>{
    const fileData=async(file,isImage)=>{if(!isImage)return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(reader.error);reader.readAsDataURL(file);});$('#storeSaveState').textContent='กำลังบีบอัดรูป';const prepared=await prepareImageForStorage(file,{maxSide:1600,maxBytes:550000});return uploadMediaImage(prepared,'products',status=>{$('#storeSaveState').textContent=status;});};
    ...
    $('#storeAdminList').querySelectorAll('.store-editor').forEach(editor=>{const index=Number(editor.dataset.index),product=products[index],media=editor.querySelector('.store-media-fields'),labels=[...media.querySelectorAll('label')];media.insertAdjacentHTML('beforebegin','<div class="store-media-guide"><b>คลังสื่อสินค้า</b><span>อัปโหลดไฟล์จากเครื่อง หรือวาง URL อย่างใดอย่างหนึ่ง</span></div>');labels.forEach((label,mediaIndex)=>{const config=[{accept:'image/*',multiple:false,text:'เลือกรูปสินค้า'},{accept:'video/*',multiple:false,text:'เลือกวิดีโอ'},{accept:'image/*',multiple:true,text:'เลือกภาพสไลด์'}][mediaIndex],input=document.createElement('input'),button=document.createElement('button');input.type='file';input.accept=config.accept;input.multiple=config.multiple;input.hidden=true;button.type='button';button.className='store-file-button';button.textContent=`＋ ${config.text}`;label.append(button,input);button.addEventListener('click',()=>input.click());input.addEventListener('change',async()=>{const files=[...input.files];if(!files.length)return;if(mediaIndex===1&&files[0].size>1500000){siteAlert('ไฟล์วิดีโอใหญ่เกินไปสำหรับฐานข้อมูลนี้ กรุณาวาง URL วิดีโอแทน (หรือเลือกไฟล์ไม่เกิน 1.5 MB)',{theme:'store',title:'วิดีโอใหญ่เกินไป'});input.value='';return;}button.textContent='กำลังเตรียมไฟล์…';try{const values=await Promise.all(files.map(file=>fileData(file,mediaIndex!==1)));if(mediaIndex===0){product.image=values[0];product.mediaCrops={...(product.mediaCrops||{}),image:{x:50,y:50,zoom:1}};}else if(mediaIndex===1)product.video=values[0];else{product.lookImages=[...(product.lookImages||[]),...values].slice(0,10);product.lookImage=product.lookImages[0]||product.lookImage;}$('#storeSaveState').textContent='มีไฟล์ใหม่ที่ยังไม่ได้บันทึก';draw();}catch(_){siteAlert('ไม่สามารถอ่านไฟล์นี้ได้ กรุณาเลือกไฟล์ใหม่',{theme:'store'});}});});const count=(product.lookImages||[]).length;if(product.image||product.video||count){const status=document.createElement('div');status.className='store-media-status';status.innerHTML=`<b>ไฟล์พร้อมใช้งาน</b>${product.image?'<span>✓ รูปสินค้า</span>':''}${product.video?'<span>✓ วิดีโอ</span>':''}${count?`<span>✓ Finish Look ${count} ภาพ</span>`:''}<button type="button">ล้างไฟล์</button>`;media.after(status);status.querySelector('button').addEventListener('click',async()=>{if(!await siteConfirm('ต้องการล้างรูปสินค้า วิดีโอ และภาพ Finish Look ทั้งหมดหรือไม่?',{theme:'store',title:'ล้างไฟล์สินค้านี้',confirmText:'ล้างไฟล์ทั้งหมด'}))return;product.image='';product.video='';product.lookImage='';product.lookImages=[];product.mediaCrops={};draw();});}});
...
async function saveStandAssignment(seat,student){
  if(!firebaseReady||!firebaseSyncRoot)throw Object.assign(new Error('Firebase is not ready'),{code:'firebase/not-ready'});
  if(!firebaseAuthUser)throw Object.assign(new Error('Admin authentication is required'),{code:'auth/admin-required'});
  const key='phanuang-stand-config',ref=firebaseSyncRoot.child(firebaseKey(key));let duplicateSeat='';
  const result=await ref.transaction(cloudRecord=>{let config;try{config={rows:10,columns:18,assignments:{},...JSON.parse(cloudRecord?.value||'{}')}}catch(_){config={rows:10,columns:18,assignments:{}}}config.assignments=config.assignments||{};if(student){duplicateSeat=Object.keys(config.assignments).find(code=>code!==seat&&config.assignments[code]?.id===student.id)||'';if(duplicateSeat)return;config.assignments[seat]=student}else delete config.assignments[seat];return {value:JSON.stringify(config),updatedAt:firebase.database.ServerValue.TIMESTAMP}},undefined,false);
  if(!result.committed){if(duplicateSeat)return {saved:false,duplicateSeat};throw new Error('บันทึกที่นั่งไม่สำเร็จ')}
  const serialized=result.snapshot.val()?.value||'{}';firebaseManualWriteKeys.add(key);try{localStorage.setItem(key,serialized)}finally{firebaseManualWriteKeys.delete(key)}return {saved:true,duplicateSeat:''};
}
...
function renderHomeStandMap() {
  ...
  host.innerHTML = `<div class="home-stand-stage"><span></span><b>STAGE · เวทีกลาง</b><span></span></div><div class="home-stand-scroll"><div class="home-stand-grid" style="--home-columns:${config.columns};--home-min-width:${Math.max(620, config.columns * 62)}px">${seatCodes.map((code,index) => { const student=assignments[code],nickname=student?.nickname && student.nickname !== '—' ? student.nickname : student?.name?.split(' ')[0]; return `<article class="home-seat ${student?'is-assigned':'is-empty'}" title="${student?`${escapeAttribute(student.name)} · ม.${escapeAttribute(student.room)} เลขที่ ${escapeAttribute(student.number)}`:'ยังไม่มีผู้ประจำที่นั่ง'}"><small>${String.fromCharCode(65 + Math.floor(index/config.columns))}</small><b>${code}</b><span>${student?escapeHTML(nickname):'ว่าง'}</span>${student?`<em>ม.${escapeHTML(student.room||'—')}</em>`:''}</article>`; }).join('')}</div></div><footer><div><span class="assigned"><i></i> มีผู้ประจำที่นั่ง</span><span><i></i> ที่นั่งว่าง</span></div><p><b>${assignedCount}</b> / ${seatCodes.length} ที่นั่งถูกจัดแล้ว</p></footer>`;
}
renderHomeStandMap();
const HOME_ROSTER_GROUPS=['บาสเกตบอล','วอลเลย์บอล','แบดมินตัน','เปตอง','เซปักตะกร้อ','กรีฑา','ฟุตบอล','กระโดดไกล','ขว้างจักร','ฟุตซอล','ดรัมเมเยอร์','เชียร์ลีดเดอร์','ขบวนพาเหรด','คัตเอาท์','สวัสดิการ/พยาบาล'];
const HOME_ROSTER_ICONS={'บาสเกตบอล':'🏀','วอลเลย์บอล':'🏐','แบดมินตัน':'🏸','เปตอง':'🎯','เซปักตะกร้อ':'<img src="assets/takraw-icon-white.png" alt="">','กรีฑา':'🏃','ฟุตบอล':'⚽','กระโดดไกล':'↗','ขว้างจักร':'◎','ฟุตซอล':'🥅','ดรัมเมเยอร์':'♬','เชียร์ลีดเดอร์':'✦','ขบวนพาเหรด':'⚑','คัตเอาท์':'▰','สวัสดิการ/พยาบาล':'✚'};
let homeRosterActiveGroup='';
function renderHomeTeamDirectory(){
  const host=$('#homeTeamDirectory');if(!host)return;let rosters={};try{rosters=JSON.parse(localStorage.getItem('phanuang-activity-attendance')||'{}')?.rosters||{}}catch(_){}
  const groups=HOME_ROSTER_GROUPS;
  if(!groups.includes(homeRosterActiveGroup))homeRosterActiveGroup=groups[0];
  const members=rosters[homeRosterActiveGroup]||[],total=HOME_ROSTER_GROUPS.reduce((sum,group)=>sum+(rosters[group]||[]).length,0);
  host.innerHTML=`<div class="home-roster-overview"><div><span>รวมทุกฝ่าย</span><b>${total}</b><small>ตำแหน่ง</small></div><p>เลือกทีมเพื่อดูรายชื่อ<br><small>ตัวเลขด้านขวาคือจำนวนสมาชิก</small></p></div><nav class="home-roster-teams" aria-label="เลือกทีม">${groups.map(group=>`<button type="button" data-home-roster="${escapeAttribute(group)}" class="${group===homeRosterActiveGroup?'active':''}"><i>${HOME_ROSTER_ICONS[group]||'●'}</i><span>${escapeHTML(group)}</span><b>${(rosters[group]||[]).length}</b></button>`).join('')}</nav><section class="home-roster-sheet"><header><div class="home-roster-current"><span>${HOME_ROSTER_ICONS[homeRosterActiveGroup]||'●'}</span><div><small>รายชื่อประจำทีม</small><h3>${escapeHTML(homeRosterActiveGroup)}</h3></div></div><label class="home-roster-search"><span>⌕</span><input type="search" id="homeRosterSearch" placeholder="ค้นหาชื่อ ชื่อเล่น ห้อง หรือเลขที่" ${members.length?'':'disabled'}></label><b id="homeRosterVisibleCount">${members.length} คน</b></header>${members.length?`<div class="home-roster-table"><div class="home-roster-table-head"><span>#</span><span>ชื่อ–นามสกุล</span><span>ชื่อเล่น</span><span>ชั้น / ห้อง</span><span>เลขที่</span></div>${members.map((student,index)=>`<div class="home-roster-row" data-roster-search="${escapeAttribute(`${student.name||''} ${student.nickname||''} ${student.room||''} ${student.number||''}`.toLowerCase())}"><i>${String(index+1).padStart(2,'0')}</i><b>${escapeHTML(student.name||'ไม่ระบุชื่อ')}</b><span>${student.nickname&&student.nickname!=='—'?escapeHTML(student.nickname):'—'}</span><span>ม.${escapeHTML(student.room||'—')}</span><span>${escapeHTML(student.number||'—')}</span></div>`).join('')}<div class="home-roster-no-result" hidden>ไม่พบรายชื่อที่ค้นหา</div></div>`:`<div class="home-roster-empty"><span>${HOME_ROSTER_ICONS[homeRosterActiveGroup]||'○'}</span><b>ยังไม่มีรายชื่อ ${escapeHTML(homeRosterActiveGroup)}</b><small>เมื่อผู้ดูแลบันทึกรายชื่อ ข้อมูลจะแสดงที่นี่อัตโนมัติ</small></div>`}</section>`;
  host.querySelectorAll('[data-home-roster]').forEach(button=>button.onclick=()=>{homeRosterActiveGroup=button.dataset.homeRoster;renderHomeTeamDirectory()});
  $('#homeRosterSearch')?.addEventListener('input',event=>{const query=event.target.value.trim().toLowerCase(),rows=[...host.querySelectorAll('.home-roster-row')];let visible=0;rows.forEach(row=>{const show=!query||row.dataset.rosterSearch.includes(query);row.hidden=!show;if(show)visible++});$('#homeRosterVisibleCount').textContent=`${visible} คน`;const empty=host.querySelector('.home-roster-no-result');if(empty)empty.hidden=visible>0});
}
renderHomeTeamDirectory();
...
// A realtime update must not destroy attendance choices that this browser has
// not submitted yet. The flag is cleared whenever the activity panel is drawn.
let activityAttendanceDraftDirty = false;
...
function renderAdmin() {
  ...
  host.innerHTML = `<div class="admin-workspace"><header class="admin-top"><div class="staff-avatar">${staff.name.slice(0,1)}</div><div><small>${staff.role==='teacher'?'สิทธิ์ครูที่ปรึกษา':'สิทธิ์แอดมินนักเรียน'} · ${escapeHTML(staff.room||'-')}</small><p><b>${staff.name}</b> <span>พร้อมใช้งาน</span></p></div><div class="admin-live" title="Database: ${escapeAttribute(firebaseHint)} · Permission: ${firebaseAdminPermission}"><i></i> ${firebaseLive ? 'เชื่อมต่อข้อมูลแล้ว' : 'ออฟไลน์'}</div><button class="admin-help-button" id="adminHome" type="button">⌂ หน้าช่วยเหลือ</button><button class="mini" id="adminLogout">ออกจากระบบ</button></header>${firebaseLive ? (firebaseAdminPermission==='ADMIN WRITE' ? '' : `<p class="admin-system-warning">⚠ เชื่อมต่อแล้ว แต่บัญชีนี้ดูข้อมูลได้อย่างเดียว</p>`) : `<p class="admin-system-warning">⚠ ยังเชื่อมต่อฐานข้อมูลไม่ได้ · ${escapeHTML(firebaseHint)}</p>`}<div class="admin-nav-hint"><b>เลือกงานที่ต้องการทำ</b><span>เริ่มจาก “หน้าหลัก” หากไม่แน่ใจว่าต้องเข้าเมนูไหน</span></div><nav class="admin-tabs" aria-label="เมนูผู้ดูแล"><button class="admin-tab active" data-admin-tab="overview"><i>⌂</i><span>หน้าหลัก<small>เริ่มต้นที่นี่</small></span></button><button class="admin-tab" data-admin-tab="attendance"><i>✓</i><span>เช็คชื่อแสตนด์<small>สแกน QR ที่นั่ง</small></span></button><button class="admin-tab" data-admin-tab="attendance-history"><i>◴</i><span>รายงานแสตนด์<small>ดูผล / PDF</small></span></button><button class="admin-tab" data-admin-tab="tokens"><i>★</i><span>แจกโทเค็น<small>กิจกรรมบนแสตนด์</small></span></button><button class="admin-tab" data-admin-tab="sports"><i>🏆</i><span>ผลการแข่งขัน<small>บันทึกผลกีฬา</small></span></button><button class="admin-tab" data-admin-tab="members"><i>♙</i><span>ทีมงาน<small>แก้ไขสมาชิก</small></span></button><button class="admin-tab" data-admin-tab="store"><i>◆</i><span>ร้านค้า<small>สินค้าและสต็อก</small></span></button><button class="admin-tab" data-admin-tab="orders"><i>▤</i><span>คำสั่งซื้อ<small>รายการและ CSV</small></span></button><button class="admin-tab" data-admin-tab="photos"><i>◫</i><span>รูปกิจกรรม<small>จัดการแกลเลอรี</small></span></button><button class="admin-tab" data-admin-tab="election"><i>✦</i><span>เลือกตั้ง<small>ผู้สมัครและผล</small></span></button></nav><div id="adminPanel"></div></div>`;
  const sportsTab=document.querySelector('[data-admin-tab="sports"]');
  if(sportsTab){const teamTab=document.createElement('button');teamTab.className='admin-tab';teamTab.dataset.adminTab='activity-attendance';teamTab.innerHTML='<i>♟</i><span>นักกีฬา/ขบวน<small>TEAMS</small></span>';sportsTab.before(teamTab);}
  ...
  $('#adminHome').addEventListener('click', () => document.querySelector('[data-admin-tab="overview"]').click());
  ...
  const requestedButton = document.querySelector(`.admin-tab[data-admin-tab="${requestedTab}"]`) || document.querySelector('.admin-tab[data-admin-tab="overview"]');
  document.querySelectorAll('.admin-tab').forEach((item) => item.classList.toggle('active', item === requestedButton));
  renderAdminPanel(requestedButton.dataset.adminTab);
}
function renderAdminOverview(host) {
  const staff=JSON.parse(localStorage.getItem('phanuang-admin')||'{}'),teamNames=Array.isArray(staff.teams)&&staff.teams.length?staff.teams.join(' · '):'ทุกฝ่ายที่ได้รับมอบหมาย';
  const jobs=[
    ['activity-attendance','♟','เช็คชื่อนักกีฬา / ขบวน','เลือกฝ่าย → ติ๊กคนที่มา → ตรวจวันที่ → กดบันทึก','งานที่ใช้บ่อย'],
    ['attendance','✓','เช็คชื่อขึ้นแสตนด์','เปิดเครื่องสแกน QR หรือพิมพ์รหัสที่นั่ง แล้วบันทึกทีละคน','เช็คหน้างาน'],
    ['attendance-history','↓','ดาวน์โหลดรายงาน PDF','เลือกรอบที่ต้องการ แล้วกดดาวน์โหลด PDF','เอกสาร'],
    ['sports','🏆','บันทึกผลการแข่งขัน','เลือกชนิดกีฬา ใส่คะแนนและผู้ชนะ แล้วบันทึก','ผลกีฬา'],
    ['tokens','★','แจกโทเค็น','ตรวจช่วงกิจกรรม ค้นหาที่นั่ง แล้วแตะเพื่อให้คะแนน','แสตนด์'],
    ['orders','▤','ตรวจคำสั่งซื้อ','ดูรายการส่วนตัว/รายห้อง และส่งออกข้อมูล','ร้านค้า']
  ];
  host.innerHTML=`<div class="admin-panel admin-overview"><header class="admin-welcome"><div><small>ยินดีต้อนรับเข้าสู่ระบบผู้ดูแล</small><h3>วันนี้ต้องการทำอะไรครับ?</h3><p>เลือกการ์ดงานด้านล่างได้ทันที ระบบจะพาไปยังหน้าที่ถูกต้อง</p></div><div class="admin-account-summary"><span>บัญชีที่กำลังใช้</span><b>${escapeHTML(staff.name||'-')}</b><small>${escapeHTML(teamNames)}</small></div></header><section class="admin-three-steps"><div><i>1</i><b>เลือกงาน</b><span>เลือกจากการ์ดด้านล่าง</span></div><div><i>2</i><b>ตรวจข้อมูล</b><span>เช็กฝ่าย วันที่ และจำนวนคน</span></div><div><i>3</i><b>กดบันทึก</b><span>รอข้อความ “บันทึกสำเร็จ”</span></div></section><div class="admin-job-grid">${jobs.map(([tab,icon,title,detail,badge])=>`<button type="button" data-open-admin-job="${tab}"><i>${icon}</i><span><em>${badge}</em><b>${title}</b><small>${detail}</small></span><strong>เปิดงาน →</strong></button>`).join('')}</div><aside class="admin-safety-note"><b>ข้อควรรู้เมื่อทำงานพร้อมกันหลายคน</b><span>แต่ละคนสามารถอยู่คนละฝ่ายได้พร้อมกัน ก่อนบันทึกให้ตรวจชื่อฝ่ายและวันที่อีกครั้ง และอย่าออกจากระบบระหว่างที่ปุ่มแสดง “กำลังบันทึก…”</span></aside></div>`;
  host.querySelectorAll('[data-open-admin-job]').forEach(button=>button.addEventListener('click',()=>document.querySelector(`[data-admin-tab="${button.dataset.openAdminJob}"]`)?.click()));
}
...
function renderAdminPanel(tab) {
  ...
  $('#admin')?.classList.toggle('activity-fast-mode',tab==='activity-attendance');
  if (tab === 'overview') { renderAdminOverview(host); return; }
  if (tab === 'activity-attendance') { renderActivityAttendance(host); return; }
  ...
  if (tab === 'tokens') { renderTokenAdminV3(host); return; }
  ...
  if (tab === 'attendance') {
    ...
    const roundLocked = Boolean((config.sessionId || config.id) && !config.finalizedAt);
    ...
    host.innerHTML = `<div class="admin-panel attendance-panel"><div class="panel-heading"><div><small>LIVE OPERATIONS · 01</small><h3>เช็คชื่อขึ้นแสตนด์</h3><p>หนึ่งช่วงเวลาจะมีรอบที่รับเช็กชื่อได้เพียงรอบเดียว</p></div><button class="primary scan-button" id="scanSeat"><span>⌗</span> สแกน QR / พิมพ์รหัส</button></div><section class="attendance-scheduler ${['open','waiting'].includes(status.state)?'is-locked':''}"><div class="scheduler-head"><div><small>AUTO SCHEDULE</small><h4>${['open','waiting'].includes(status.state)?`รอบที่ ${config.round||'—'} ถูกล็อกอยู่`:'กำหนดรอบเช็คชื่อถัดไป'}</h4></div><span>${status.state==='open'?'กำลังรับเช็กชื่อ':status.state==='waiting'?'รอเวลาเปิด':'พร้อมตั้งรอบใหม่'}</span></div><div class="attendance-config"><div class="field schedule-date"><label>วันที่เช็คชื่อ</label><input id="attendanceDate" type="date" value="${startDate}"></div><div class="field"><label>เวลาเริ่ม</label><input id="attendanceStartClock" type="time" value="${startClock}"></div><div class="field"><label>เปิดรับนาน</label><select id="attendanceDuration"><option value="10" ${savedDuration===10?'selected':''}>10 นาที</option><option value="15" ${savedDuration===15?'selected':''}>15 นาที</option><option value="30" ${savedDuration===30?'selected':''}>30 นาที</option><option value="45" ${savedDuration===45?'selected':''}>45 นาที</option><option value="60" ${savedDuration===60?'selected':''}>1 ชั่วโมง</option><option value="${savedDuration}" ${![10,15,30,45,60].includes(savedDuration)?'selected':''}>${savedDuration} นาที</option></select></div><button class="primary save-attendance-session" id="saveAttendanceTime">${['open','waiting'].includes(status.state)?`รอบที่ ${config.round||'—'} กำลังทำงาน`:'ตั้งเวลาเปิดอัตโนมัติ <b>→</b>'}</button></div><div class="schedule-preview" id="attendanceSchedulePreview"></div></section><div class="attendance-active-round"><span>รอบที่รับเช็กชื่อจริง</span><b>${config.round?`รอบที่ ${config.round}`:'ยังไม่มีรอบ'}</b><small>${config.time?formatAttendanceSession(config):'กรุณาตั้งรอบก่อนเริ่มเช็กชื่อ'}</small></div><div class="attendance-status state-${status.state}"><i></i><div><b id="attendanceStatusTitle"></b><span id="attendanceStatusMessage"></span></div><strong id="attendanceStatusCountdown"></strong></div><div class="attendance-history"><label for="attendanceSessionSelect">เลือกรอบเพื่อดูผังย้อนหลัง <small>การเลือกตรงนี้ไม่เปลี่ยนรอบที่รับเช็กชื่อ</small></label><select id="attendanceSessionSelect">${sessions.length ? sessions.map((session) => `<option value="${session.id}" ${session.id === attendanceViewingSessionId ? 'selected' : ''}>${formatAttendanceSession(session)}</option>`).join('') : '<option value="">ยังไม่มีรอบที่บันทึก</option>'}</select></div><p class="scan-note"><i></i> เมื่อกดสแกน ระบบจะกลับไปยังรอบที่กำลังรับเช็กชื่อโดยอัตโนมัติ</p><div class="stand-screen"><div class="stand-screen-head"><div><small>GRANDSTAND · READ ONLY MAP</small><b>ผังที่นั่งแสตนด์</b><em id="attendanceMapSessionLabel"></em></div><span id="attendanceSummary"></span></div><div class="seat-map" id="seatMap"></div><div class="seat-legend"><span><i class="present"></i> มาแล้ว</span><span><i></i> ยังไม่มา</span></div></div></div><dialog class="attendance-scan-dialog" id="attendanceScanDialog"><div class="scan-dialog-head"><div><small>SECURE CHECK-IN · <span id="attendanceScanRound"></span></small><h3>สแกน QR ที่นั่ง</h3></div><button type="button" id="closeAttendanceScanner" aria-label="ปิด">×</button></div><div class="camera-stage"><video id="attendanceCamera" playsinline muted></video><div class="camera-reticle"><i></i><i></i><i></i><i></i></div><div class="camera-message" id="attendanceCameraMessage">กำลังเตรียมกล้อง…</div></div><div class="manual-checkin"><span>หรือพิมพ์รหัสที่นั่ง</span><form id="manualSeatForm"><input id="manualSeatCode" autocomplete="off" placeholder="เช่น A1" maxlength="3"><button class="primary">บันทึกเช็คชื่อ →</button></form><div class="error" id="attendanceScanError" aria-live="polite"></div></div></dialog>`;
    ...
    if (roundLocked) document.querySelectorAll('.attendance-scheduler input,.attendance-scheduler select,.attendance-scheduler button').forEach((control)=>control.disabled=true);
    if (roundLocked) {
      const activeBar=document.querySelector('.attendance-active-round');
      activeBar?.insertAdjacentHTML('beforeend','<button type="button" id="finalizeAttendanceRound">จบรอบนี้</button>');
      $('#finalizeAttendanceRound')?.addEventListener('click',finalizeAttendanceRound);
      const scheduler=document.querySelector('.attendance-scheduler');scheduler?.classList.add('is-locked');
      const heading=scheduler?.querySelector('.scheduler-head h4');if(heading)heading.textContent=`ยังเช็กรอบที่ ${config.round||'—'} อยู่`;
      const badge=scheduler?.querySelector('.scheduler-head>span');if(badge)badge.textContent='ต้องจบรอบเดิมก่อน';
    }
  ...
  if (tab === 'photos') {
    ...
    let selectedImages = gallery.images.slice(0, 200);
    host.innerHTML = `<div class="admin-panel gallery-admin-panel"><h3>รูปภาพกิจกรรม</h3><p>เพิ่ม ดูตัวอย่าง ลบ หรือจัดลำดับรูปภาพได้สูงสุด 200 รูป โดยหน้าเว็บจะโหลดเฉพาะรูปที่ใช้แสดง</p><div class="field"><label>ลิงก์ Google Drive สำหรับปุ่มดูรูปภาพกิจกรรม</label><input id="driveUrl" type="url" placeholder="https://drive.google.com/..." value="${gallery.driveUrl || ''}"></div><div class="gallery-admin-actions"><button type="button" class="upload-button" id="chooseActivityPhotos">เลือกเพิ่มรูปภาพหลายรูป</button><input id="activityPhotos" type="file" accept="image/*" multiple="multiple" hidden><button class="mini" id="saveGallery">บันทึกการตั้งค่า</button></div><p class="gallery-upload-help">ระบบจะย่อและอัปโหลดรูปขึ้นคลาวด์ทีละรูป เพื่อลดการใช้หน่วยความจำบน iPad · เปลี่ยนช่อง “ตำแหน่ง” เพื่อสลับลำดับรูป</p><p class="gallery-count" aria-live="polite"></p><div class="admin-gallery-preview" id="adminGalleryPreview"></div></div>`;
    const count = $('.gallery-count');
    const preview = $('#adminGalleryPreview');
    let uploadQueue = Promise.resolve();
    let imagesOptimized = false;
    const compressActivityImage = (file) => typeof file !== 'string' ? prepareImageForStorage(file, { maxSide: 1800, maxBytes: 650000 }) : Promise.resolve(file);
    $('#chooseActivityPhotos').addEventListener('click', () => $('#activityPhotos').click());
    const syncGallery = async () => {
      try {
        selectedImages = await uploadEmbeddedImages(selectedImages, 'gallery', (current, total, status) => { count.textContent = `${status} ${current} / ${total} รูป…`; });
        const serialized = JSON.stringify({ images: selectedImages, driveUrl: $('#driveUrl').value.trim() });
        await saveFirebaseSharedValue('phanuang-gallery', serialized);
        console.info('[Firebase] image URL saved', { key:'phanuang-gallery', imageCount:selectedImages.length });
        localStorage.setItem('phanuang-gallery', serialized);
        if ($('#gallery').classList.contains('active')) renderGalleryPage();
        return true;
      } catch (error) {
        window.alert(adminSaveErrorMessage(error));
        return false;
      }
    };
    const renderPreviews = () => {
      count.textContent = `เลือกแล้ว ${selectedImages.length} / 200 รูป`;
    ...
    $('#activityPhotos').addEventListener('change', (event) => {
      ...
      uploadQueue = uploadQueue.then(async () => {
        ...
        } catch (error) { console.error('[Cloudinary] upload failed', { code:error?.code, message:error?.message }); window.alert(error.message || adminSaveErrorMessage(error)); renderPreviews(); return; }
        const remaining = 200 - selectedImages.length;
        if (!remaining) { window.alert('เพิ่มรูปภาพครบ 200 รูปแล้ว กรุณาลบรูปเดิมก่อน'); return; }
        const chosenFiles = files.slice(0, remaining);
        count.textContent = `กำลังบีบอัดรูป 1 / ${chosenFiles.length}…`;
        try {
          for (let index = 0; index < chosenFiles.length; index += 1) {
            count.textContent = `กำลังบีบอัดรูป ${index + 1} / ${chosenFiles.length} รูป…`;
            selectedImages.push(await uploadMediaImage(await compressActivityImage(chosenFiles[index]), 'gallery', status => { count.textContent = `${status} ${index + 1} / ${chosenFiles.length} รูป…`; }));
          }
          renderPreviews();
          if (files.length > remaining) window.alert(`เพิ่มได้อีก ${remaining} รูป จึงเลือกเฉพาะ ${remaining} รูปแรก`);
        } catch (error) { window.alert(error.message || adminSaveErrorMessage(error)); renderPreviews(); }
      });
    });
    $('#saveGallery').addEventListener('click', async () => { const button = $('#saveGallery'); button.disabled = true; try { if (await syncGallery()) window.alert('บันทึกการตั้งค่ารูปภาพกิจกรรมแล้ว'); } finally { button.disabled = false; renderPreviews(); } });
    renderPreviews();
    return;
  }
...
async function renderCommitteeAdmin(host) {
  ...
  // Do not decode every saved photo while opening admin; legacy Base64 images are
  // migrated to Cloudinary sequentially only when Save is pressed.
  ...
  const bindRows = () => { list.querySelectorAll('.member-editor-row').forEach((el) => { el.querySelector('.member-delete').addEventListener('click', () => { syncFromRows(); members.splice(Number(el.dataset.index),1); renderRows(); }); el.querySelector('input[type=file]').addEventListener('change', async (event) => { const file = event.target.files[0]; if (!file) return; const memberIndex = Number(el.dataset.index),previousImage=members[memberIndex]?.image||''; try { $('#memberSaveStatus').textContent='กำลังบีบอัดรูป'; const prepared = await prepareImageForStorage(file, { maxSide: 1200, maxBytes: 350000 }); syncFromRows(); if (!members[memberIndex]) return; openMemberCropStudio(prepared, async (croppedImage) => { if (!members[memberIndex]) return; try { const secureUrl=await uploadMediaImage(croppedImage,'members',status=>{$('#memberSaveStatus').textContent=status;}); members[memberIndex].image=secureUrl; renderRows(); $('#memberSaveStatus').textContent='อัปโหลดสำเร็จ · กดบันทึกและเผยแพร่'; } catch(error) { members[memberIndex].image=previousImage; renderRows(); $('#memberSaveStatus').textContent='อัปโหลดไม่สำเร็จ'; window.alert(adminSaveErrorMessage(error)); } }); } catch (error) { members[memberIndex].image=previousImage; window.alert(error.message || 'ไม่สามารถอ่านรูปภาพนี้ได้'); } }); }); };
  ...
  $('#saveCommitteeMembers').addEventListener('click', async () => { syncFromRows(); const button = $('#saveCommitteeMembers'); button.disabled = true; $('#memberSaveStatus').textContent = 'กำลังบีบอัดรูป'; try { const nextImages=[]; for (let index = 0; index < members.length; index += 1) nextImages.push(await uploadMediaImage(members[index].image, 'members',status=>{$('#memberSaveStatus').textContent=`${status} ${index + 1} / ${members.length}`;})); nextImages.forEach((image,index)=>{members[index].image=image;}); const serialized = JSON.stringify(members); await saveFirebaseSharedValue(COMMITTEE_KEY, serialized); console.info('[Firebase] image URL saved',{key:COMMITTEE_KEY,memberCount:members.length}); await setCommitteeMembers(members); localStorage.setItem(COMMITTEE_KEY, serialized); renderCommitteeMembers(true); $('#memberSaveStatus').textContent = `บันทึกแล้ว ${members.length} การ์ด`; } catch (error) { $('#memberSaveStatus').textContent = 'อัปโหลดไม่สำเร็จ'; window.alert(adminSaveErrorMessage(error)); } finally { button.disabled = false; } });
  renderRows();
}
...
function bindCandidateEditorRow(row) {
  ...
  row.querySelector('.candidate-image').addEventListener('change', async (event) => {
    const file = event.target.files[0]; if (!file) return;
    try { const prepared = await prepareImageForStorage(file, { maxSide: 1400, maxBytes: 320000 }); row.querySelector('.candidate-image-data').value = prepared; const x = row.querySelector('.intro-image-x').value, y = row.querySelector('.intro-image-y').value, zoom = row.querySelector('.intro-image-zoom').value; row.querySelector('.candidate-editor-photo').innerHTML = `<img src="${prepared}" style="object-position:${x}% ${y}%;transform:scale(${zoom})" alt=""><input class="candidate-image" type="file" accept="image/*" aria-label="รูปผู้สมัคร"><input class="candidate-image-data" type="hidden" value="${prepared}">`; bindCandidateEditorRowImage(row); } catch (error) { window.alert(error.message || 'ไม่สามารถอ่านรูปภาพนี้ได้'); }
  });
}
...
function bindCandidateEditorRowImage(row) {
  ...
  row.querySelector('.candidate-image').addEventListener('change', async (event) => {
    const file = event.target.files[0]; if (!file) return;
    try { const prepared = await prepareImageForStorage(file, { maxSide: 1400, maxBytes: 320000 }); row.querySelector('img').src = prepared; row.querySelector('.candidate-image-data').value = prepared; } catch (error) { window.alert(error.message || 'ไม่สามารถอ่านรูปภาพนี้ได้'); }
  });
}
async function saveElectionAdmin() {
  ...
  try {
    await Promise.all(config.candidates.map(async (candidate) => { candidate.image = await shrinkLegacyImage(candidate.image, { maxSide: 1400, maxBytes: 320000 }); }));
    ...
    message.textContent = adminSaveErrorMessage(error);
    message.classList.add('error');
  } finally {
...
async function finalizeAttendanceRound() {
  const config=getAttendanceConfig(),sessionId=config.sessionId||config.id;if(!sessionId||config.finalizedAt)return;
  const attendance=getAttendanceRecords()[sessionId]||{},confirmed=await siteConfirm(`กำลังเช็กรอบที่ ${config.round||'ปัจจุบัน'} อยู่ มีผู้เช็กชื่อแล้ว ${Object.keys(attendance).length} คน\n\nยืนยันจบรอบนี้เพื่อเปิดให้สร้างรอบใหม่หรือไม่?`,{theme:'attendance',title:`จบรอบที่ ${config.round||'ปัจจุบัน'}`,confirmText:'ยืนยันจบรอบ',danger:false});
  if(!confirmed)return;
  const finalizedAt=new Date().toISOString(),nextConfig={...config,finalizedAt},sessions=getAttendanceSessions().map(session=>session.id===sessionId?{...session,finalizedAt}:session);
  const configValue=JSON.stringify(nextConfig),sessionsValue=JSON.stringify(sessions),button=$('#finalizeAttendanceRound');if(button){button.disabled=true;button.textContent='กำลังจบรอบ…'}
  try{await saveFirebaseSharedValue('phanuang-attendance-config',configValue);await saveFirebaseSharedValue(ATTENDANCE_SESSIONS_KEY,sessionsValue);firebaseManualWriteKeys.add('phanuang-attendance-config');firebaseManualWriteKeys.add(ATTENDANCE_SESSIONS_KEY);localStorage.setItem('phanuang-attendance-config',configValue);localStorage.setItem(ATTENDANCE_SESSIONS_KEY,sessionsValue);showAdminToast(`จบรอบที่ ${config.round||'ปัจจุบัน'} แล้ว`,'สามารถตั้งรอบถัดไปได้แล้ว');renderAdminPanel('attendance')}catch(error){if(button){button.disabled=false;button.textContent='จบรอบนี้'}window.alert(adminSaveErrorMessage(error))}finally{firebaseManualWriteKeys.delete('phanuang-attendance-config');firebaseManualWriteKeys.delete(ATTENDANCE_SESSIONS_KEY)}
}
function saveAttendanceSession() {
  const currentConfig = getAttendanceConfig();
  if ((currentConfig.sessionId || currentConfig.id) && !currentConfig.finalizedAt) {
    window.alert(`ยังเปิดรอบใหม่ไม่ได้ เพราะยังเช็กรอบที่ ${currentConfig.round||'ปัจจุบัน'} อยู่ กรุณากด “จบรอบนี้” ก่อน`);
    return;
  }
...
function renderAttendanceHistory(host) {
  ...
  $('#printAttendanceReport').textContent='↓ ดาวน์โหลด PDF';
  document.querySelector('.history-panel-head p').textContent='ตรวจสอบผลแต่ละรอบและดาวน์โหลดรายงานฉบับสมบูรณ์เป็น PDF';
...
function preparePdfWindow(pdfWindow, filename, orientation='portrait', elementSelector='body') {
  if(!pdfWindow)return;
  const script=pdfWindow.document.createElement('script');
  script.src='https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js';
  pdfWindow.document.head.appendChild(script);
  pdfWindow.print=async()=>{
    const started=Date.now();
    while(!pdfWindow.html2pdf&&Date.now()-started<15000)await new Promise(resolve=>setTimeout(resolve,100));
    if(!pdfWindow.html2pdf){pdfWindow.alert('โหลดระบบสร้าง PDF ไม่สำเร็จ กรุณาตรวจสอบอินเทอร์เน็ตแล้วลองใหม่');return}
    const target=pdfWindow.document.querySelector(elementSelector)||pdfWindow.document.body,tools=pdfWindow.document.querySelector('.print-tools');
    if(tools)tools.style.display='none';
    try{await pdfWindow.html2pdf().set({margin:0,filename,image:{type:'jpeg',quality:.96},html2canvas:{scale:2,useCORS:true,logging:false},jsPDF:{unit:'mm',format:'a4',orientation},pagebreak:{mode:['css','legacy']}}).from(target).save()}
    finally{if(tools)tools.style.display='flex'}
  };
}
function printAttendanceReport() {
  refreshSeatCodes();const session=getAttendanceSessions().find(item=>item.id===attendanceViewingSessionId);if(!session)return;
  const attendance=getAttendance(session.id),config=getStandConfig(),assignments=config.assignments||{},present=seatCodes.filter(code=>attendance[code]),absent=seatCodes.filter(code=>!attendance[code]),percent=seatCodes.length?Math.round(present.length/seatCodes.length*100):0;
  const seatCard=code=>{const student=assignments[code],record=attendance[code];return `<div class="seat ${record?'present':'absent'}"><b>${code}</b><span>${student?escapeHTML(student.nickname&&student.nickname!=='—'?student.nickname:student.name.split(' ')[0]):'ว่าง'}</span>${record?`<small>${new Date(record.time).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})}</small>`:'<small>—</small>'}</div>`};
  const personRow=(code,index)=>{const student=assignments[code],record=attendance[code];return `<tr><td>${index+1}</td><td><b>${code}</b></td><td>${student?escapeHTML(student.name):'<span class="muted">ยังไม่ผูกนักเรียน</span>'}${student?.nickname&&student.nickname!=='—'?`<small>${escapeHTML(student.nickname)}</small>`:''}</td><td>${student?`ม.${student.room}`:'—'}</td><td>${student?.number||'—'}</td><td>${record?new Date(record.time).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'}):'—'}</td><td><span class="tag ${record?'yes':'no'}">${record?'มา':'ขาด'}</span></td></tr>`};
  ...
  printWindow.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>รายงานเช็คชื่อแสตนด์</title><link href="https://fonts.googleapis.com/css2?family=Kanit:wght@400;500;600;700&display=swap" rel="stylesheet"><style>@page{size:A4 landscape;margin:10mm}*{box-sizing:border-box}body{margin:0;color:#1f2630;font-family:Kanit,sans-serif}.hero{display:flex;align-items:center;justify-content:space-between;padding-bottom:5mm;border-bottom:3px solid #e5b62d}.hero small{color:#96700c;letter-spacing:2px}.hero h1{margin:1mm 0;font-size:21pt}.hero p{margin:0;color:#69717d}.mark{width:20mm;height:20mm;display:grid;place-items:center;border:2px solid #d8a920;border-radius:50%;color:#9a7100;font-size:18pt;font-weight:700}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm;margin:5mm 0}.summary div{padding:3mm 4mm;border:1px solid #d4d8dd;border-radius:3mm;background:#f5f6f7}.summary span,.summary small{display:block;color:#747d89;font-size:8pt}.summary b{display:block;font-size:19pt}.summary .yes{border-color:#75b89e;background:#e3f2ec}.summary .yes b{color:#167052}.summary .no{border-color:#d39aa5;background:#f7e7ea}.summary .no b{color:#a03b4d}.map-section{padding:4mm;border:1px solid #cfd4da;border-radius:4mm;break-after:page}.section-head{display:flex;align-items:end;justify-content:space-between;margin-bottom:3mm}.section-head small{color:#8d6b13;letter-spacing:1.5px}.section-head h2{margin:0;font-size:14pt}.legend{display:flex;gap:4mm;font-size:8pt}.legend i{display:inline-block;width:3mm;height:3mm;margin-right:1mm;border-radius:1mm;background:#d78e9b}.legend i.yes{background:#49aa85}.seat-map{display:grid;grid-template-columns:repeat(${config.columns},minmax(0,1fr));gap:1.2mm}.seat{min-width:0;padding:1.2mm .7mm;border:1px solid #cf7181;border-radius:1.5mm;background:#f7dfe3;text-align:center}.seat.present{border-color:#3d9c79;background:#dcf2e9}.seat b,.seat span,.seat small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.seat b{font-size:7pt}.seat span{font-size:5.5pt}.seat small{color:#697781;font-size:4.8pt}.lists-page{padding-top:2mm}.lists-page>header{display:flex;align-items:end;justify-content:space-between;margin-bottom:4mm}.lists-page h2{margin:0;font-size:17pt}.lists-page header span{color:#6f7884}.attendance-table{margin-bottom:7mm;break-inside:auto}.attendance-table h3{margin:0 0 2mm;padding:2mm 3mm;border-left:4px solid #399975;background:#e5f2ed;font-size:12pt}.attendance-table.absent h3{border-color:#bd5367;background:#f6e5e8}.attendance-table h3 span{float:right;font-size:9pt}table{width:100%;border-collapse:collapse;font-size:8pt}thead{display:table-header-group}th{padding:2mm;background:#273243;color:#fff;text-align:left}td{padding:1.7mm 2mm;border-bottom:1px solid #dce0e4}td small{display:block;color:#777}.muted{color:#999}.tag{display:inline-block;min-width:12mm;padding:1mm;border-radius:99px;text-align:center;font-weight:600}.tag.yes{background:#dcefe7;color:#176349}.tag.no{background:#f4dce1;color:#9b3448}.foot{margin-top:5mm;padding-top:2mm;border-top:1px solid #ccd1d7;color:#777;font-size:7pt;text-align:right}@media print{.map-section{break-after:page}.attendance-table{break-inside:auto}tr{break-inside:avoid}}</style></head><body><header class="hero"><div><small>PHUNRUEANG · GRANDSTAND ATTENDANCE</small><h1>รายงานเช็คชื่อขึ้นแสตนด์</h1><p>${formatAttendanceSession(session)}</p></div><div class="mark">PR</div></header><section class="summary"><div><span>ที่นั่งทั้งหมด</span><b>${seatCodes.length}</b><small>${config.rows} แถว × ${config.columns} ที่นั่ง</small></div><div class="yes"><span>มาแล้ว</span><b>${present.length}</b><small>${percent}% ของทั้งหมด</small></div><div class="no"><span>ยังไม่มา</span><b>${absent.length}</b><small>${100-percent}% ของทั้งหมด</small></div><div><span>เวลาเช็คชื่อ</span><b>${new Date(session.time).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})}</b><small>ถึง ${new Date(session.close).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})} น.</small></div></section><section class="map-section"><div class="section-head"><div><small>GRANDSTAND MAP</small><h2>ผังที่นั่งตามแถวจริง</h2></div><div class="legend"><span><i class="yes"></i>มาแล้ว</span><span><i></i>ยังไม่มา</span></div></div><div class="seat-map">${seatCodes.map(seatCard).join('')}</div></section><main class="lists-page"><header><div><small>ATTENDANCE DETAILS</small><h2>รายชื่อประจำรอบ</h2></div><span>มา ${present.length} · ขาด ${absent.length} · รวม ${seatCodes.length}</span></header><section class="attendance-table"><h3>✓ รายชื่อที่มาแล้ว <span>${present.length} คน</span></h3><table><thead><tr><th>#</th><th>ที่นั่ง</th><th>ชื่อ–นามสกุล / ชื่อเล่น</th><th>ชั้น</th><th>เลขที่</th><th>เวลา</th><th>สถานะ</th></tr></thead><tbody>${present.map(personRow).join('')||'<tr><td colspan="7">ยังไม่มีผู้มา</td></tr>'}</tbody></table></section><section class="attendance-table absent"><h3>× รายชื่อที่ยังไม่มา <span>${absent.length} คน</span></h3><table><thead><tr><th>#</th><th>ที่นั่ง</th><th>ชื่อ–นามสกุล / ชื่อเล่น</th><th>ชั้น</th><th>เลขที่</th><th>เวลา</th><th>สถานะ</th></tr></thead><tbody>${absent.map(personRow).join('')||'<tr><td colspan="7">ไม่มีผู้ขาด</td></tr>'}</tbody></table></section></main><footer class="foot">พิมพ์จากระบบ PHUNRUEANG STAFF CONTROL · ${new Date().toLocaleString('th-TH',{dateStyle:'long',timeStyle:'short'})}</footer><script>addEventListener('load',()=>setTimeout(()=>print(),350))<\/script></body></html>`);printWindow.document.close();
  const onePageStyle=printWindow.document.createElement('style');onePageStyle.textContent=`@page{size:A4 landscape;margin:6mm}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}html,body{width:285mm;height:198mm;overflow:hidden}.hero{height:19mm;padding-bottom:2mm}.hero h1{font-size:17pt}.hero .mark{width:16mm;height:16mm;font-size:14pt}.summary{height:24mm;margin:3mm 0;gap:2mm}.summary div{padding:2mm 3mm}.summary b{font-size:15pt}.map-section{height:143mm;padding:3mm;overflow:hidden;break-after:auto!important}.section-head{height:12mm;margin-bottom:2mm}.section-head h2{font-size:12pt}.seat-map{height:123mm;grid-template-rows:repeat(${config.rows},minmax(0,1fr));gap:.8mm}.seat{display:flex;align-items:center;justify-content:center;flex-direction:column;padding:.4mm;border:0;border-radius:1.3mm;background:#df5267!important;color:#fff!important}.seat.present{background:#27a77c!important;color:#fff!important}.seat b{font-size:6.5pt}.seat span{max-width:100%;color:#fff!important;font-size:5pt}.seat small{color:#fff!important;font-size:4.5pt;opacity:.9}.lists-page,.foot{display:none!important}`;printWindow.document.head.appendChild(onePageStyle);
  const headerFixStyle=printWindow.document.createElement('style');headerFixStyle.textContent=`.hero{height:24mm!important;padding:2mm 0!important;overflow:visible!important}.hero>div:first-child{padding-top:1mm}.hero small{display:block!important;line-height:1.5!important}.hero h1{margin:.5mm 0!important;font-size:17pt!important;line-height:1.2!important}.hero p{line-height:1.35!important}.summary{height:22mm!important;margin:2mm 0!important}.summary div{padding:1.5mm 3mm!important}.summary b{font-size:14pt!important}.map-section{height:146mm!important}.section-head{height:11mm!important}.seat-map{height:126mm!important}`;printWindow.document.head.appendChild(headerFixStyle);
  preparePdfWindow(printWindow,`attendance-stand-${session.id}.pdf`,'landscape');
}
...
function renderSeatMap() {
  ...
  const assignments=getStandConfig().assignments||{};
  const map = $('#seatMap');
  if (!map) return;
  map.style.gridTemplateColumns=`repeat(${getStandConfig().columns},minmax(38px,1fr))`;
  map.innerHTML = seatCodes.map((seat) => {const s=assignments[seat];return `<span class="seat ${attendance[seat] ? 'present' : 'absent'} ${s?'assigned':''}" data-seat="${seat}" title="${s?`${escapeAttribute(s.name)} · ม.${s.room} เลขที่ ${s.number}`:'ยังไม่ได้ผูกนักเรียน'}${attendance[seat] ? ` · เช็คชื่อ ${new Date(attendance[seat].time).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})} น.` : ''}"><b>${seat}</b>${s?`<small>${escapeHTML(s.nickname==='—'?s.name.split(' ')[0]:s.nickname)}</small><em>ม.${escapeHTML(s.room||'—')}</em>`:''}</span>`}).join('');
  const present = Object.keys(attendance).length;
  $('#attendanceSummary').textContent = `มา ${present} / ${seatCodes.length}`;
  const viewedSession=getAttendanceSessions().find(session=>session.id===attendanceViewingSessionId);
  const activeId=getAttendanceConfig().sessionId||getAttendanceConfig().id;
  const label=$('#attendanceMapSessionLabel');if(label)label.textContent=viewedSession?`${viewedSession.id===activeId?'● รอบที่กำลังรับเช็กชื่อ':'○ กำลังดูย้อนหลัง'} · ${formatAttendanceSession(viewedSession)}`:'ยังไม่ได้เลือกรอบ';
}
async function openAttendanceScanner() {
  const config=getAttendanceConfig(),status = getAttendanceWindowStatus(config);
  const dialog = $('#attendanceScanDialog');
  const error = $('#attendanceScanError');
  error.classList.remove('success-message'); error.textContent = '';
  attendanceViewingSessionId=config.sessionId||config.id||'';
  renderSeatMap();
  const roundLabel=$('#attendanceScanRound');if(roundLabel)roundLabel.textContent=config.round?`รอบที่ ${config.round}`:'ยังไม่มีรอบ';
  dialog.showModal();
...
async function printAllSeatQrs() {
  ...
  printWindow.document.open();
  printWindow.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>QR ที่นั่งทั้งหมด</title><style>@page{size:A4 portrait;margin:8mm}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}html{background:#eef1f4}body{max-width:210mm;margin:0 auto;color:#202733;background:#fff;font-family:Arial,"Tahoma",sans-serif}.sheet{display:grid;grid-template-columns:repeat(3,1fr);gap:3mm;padding:8mm}article{height:65mm;display:flex;align-items:center;justify-content:center;flex-direction:column;padding:3mm;border:1px dashed #8b939d;break-inside:avoid;page-break-inside:avoid;text-align:center}img{display:block;width:36mm;height:36mm;object-fit:contain;image-rendering:pixelated}article div{width:100%;min-width:0}b{display:block;margin:1mm 0 0;font-size:20pt;line-height:1.1}span,small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}span{margin-top:1mm;font-size:9pt;font-weight:700}small{margin-top:.6mm;color:#596473;font-size:7pt}.print-tools{position:sticky;top:0;z-index:2;display:flex;align-items:center;justify-content:space-between;padding:12px 18px;background:#202a39;color:#fff}.print-tools span{margin:0}.print-tools small{margin:2px 0 0;color:#aeb8c5}.print-tools button{min-width:150px;padding:10px 18px;border:0;border-radius:8px;background:#e3b62d;color:#171d28;font-weight:700;cursor:pointer}.print-tools button:disabled{opacity:.65;cursor:wait}@media print{html,body{background:#fff}.print-tools{display:none}.sheet{gap:3mm;padding:0}}</style></head><body><header class="print-tools"><div><span>QR ที่นั่งทั้งหมด ${seatCodes.length} ที่นั่ง</span><small id="printHint">ตรวจสอบป้ายด้านล่าง แล้วกดพิมพ์เมื่อพร้อม</small></div><button id="printAllButton" type="button">พิมพ์ทั้งหมด</button></header><main class="sheet">${cards.join('')}</main><script>const button=document.getElementById('printAllButton'),hint=document.getElementById('printHint');button.addEventListener('click',async()=>{button.disabled=true;hint.textContent='กำลังเตรียมภาพ QR…';const images=[...document.images];await Promise.all(images.map(image=>image.complete?Promise.resolve():new Promise(resolve=>{image.addEventListener('load',resolve,{once:true});image.addEventListener('error',resolve,{once:true})})));await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));hint.textContent='พร้อมพิมพ์';button.disabled=false;window.print()});</script></body></html>`);
  printWindow.document.close();
  const fifteenPerPageStyle = printWindow.document.createElement('style');
  fifteenPerPageStyle.textContent = '.sheet{grid-auto-rows:52mm}article{height:52mm;padding:2mm}article img{width:30mm;height:30mm}@media print{article:nth-child(15n):not(:last-child){break-after:page;page-break-after:always}}';
  printWindow.document.head.appendChild(fifteenPerPageStyle);
  const pdfButton=printWindow.document.querySelector('#printAllButton'),pdfHint=printWindow.document.querySelector('#printHint');
  if(pdfButton)pdfButton.textContent='↓ ดาวน์โหลด PDF';
  if(pdfHint)pdfHint.textContent='ตรวจสอบป้ายด้านล่าง แล้วดาวน์โหลดเป็นไฟล์ PDF';
  preparePdfWindow(printWindow,'grandstand-seat-qr.pdf','portrait','.sheet');
  printWindow.focus();
}
function downloadStandRosterPdf(){
  refreshSeatCodes();const config=getStandConfig(),assignments=config.assignments||{},roster=Object.entries(assignments).filter(([,student])=>student).sort(([seatA],[seatB])=>seatA.localeCompare(seatB,undefined,{numeric:true}));
  if(!roster.length){alert('ยังไม่มีนักเรียนที่ผูกกับที่นั่ง กรุณาเพิ่มข้อมูลก่อนดาวน์โหลดใบรายชื่อ');return}
  const pdfWindow=window.open('','_blank','width=1280,height=850');if(!pdfWindow){alert('เบราว์เซอร์บล็อกหน้าต่างดาวน์โหลด PDF กรุณาอนุญาต Pop-up');return}
  const checks=Array.from({length:5},()=>'<th class="check"></th>').join(''),emptyChecks=Array.from({length:5},()=>'<td class="check"></td>').join('');
  const row=(entry,index)=>{const [seat,student]=entry||[];return `<tr><td class="number">${student?index+1:''}</td><td class="seat">${student?escapeHTML(seat):''}</td><td class="person">${student?`<b>${escapeHTML(student.name||'')}</b>${student.nickname&&student.nickname!=='—'?`<small> / ${escapeHTML(student.nickname)}</small>`:''}`:''}</td><td class="room">${student?`ม.${escapeHTML(student.room||'—')}`:''}</td>${emptyChecks}</tr>`};
  const table=start=>`<table><thead><tr><th class="number">ที่</th><th class="seat">ที่นั่ง</th><th class="person">ชื่อจริง / ชื่อเล่น</th><th class="room">ชั้น / ห้อง</th>${checks}</tr></thead><tbody>${Array.from({length:25},(_,offset)=>row(roster[start+offset],start+offset)).join('')}</tbody></table>`;
  const pageCount=Math.max(1,Math.ceil(roster.length/50)),pages=Array.from({length:pageCount},(_,pageIndex)=>{const start=pageIndex*50;return `<section class="stand-roster-page"><main>${table(start)}${table(start+25)}</main><footer><div><span>จำนวนนักเรียนที่มีรายชื่อ</span><b>${roster.length} คน</b></div><div><span>จำนวนที่นั่งทั้งหมดในผัง</span><b>${seatCodes.length} ที่นั่ง</b></div><div><span>ที่นั่งที่ยังไม่ได้ผูกนักเรียน</span><b>${Math.max(0,seatCodes.length-roster.length)} ที่นั่ง</b></div><div class="sign"><span>ลงชื่อผู้ตรวจสอบ</span><i></i></div><em>ใบรายชื่อเช็คชื่อแสตนด์ · หน้า ${pageIndex+1}/${pageCount}</em></footer></section>`}).join('');
  pdfWindow.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>ใบรายชื่อเช็คชื่อแสตนด์</title><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;500;600;700&display=swap" rel="stylesheet"><style>@page{size:A4 landscape;margin:0}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}html,body{margin:0;background:#dfe3e8;color:#17202b;font-family:Sarabun,Tahoma,sans-serif}.stand-roster-page{width:297mm;height:210mm;margin:0 auto;padding:7mm 8mm 6mm;background:#fff;break-after:page;page-break-after:always;overflow:hidden}.stand-roster-page:last-child{break-after:auto;page-break-after:auto}main{height:178mm;display:grid;grid-template-columns:1fr 1fr;gap:5mm}table{width:100%;height:100%;border-collapse:collapse;table-layout:fixed;font-size:6.8pt}th,td{height:6.45mm;padding:.7mm 1mm;border:.25mm solid #555;line-height:1.05}th{height:7mm;background:#e8e8e8;text-align:center;font-weight:700}.number{width:8mm;text-align:center}.seat{width:13mm;text-align:center;font-weight:700}.person{width:auto;text-align:left}.person b{font-weight:600}.person small{font-size:6pt}.room{width:17mm;text-align:center}.check{width:6.5mm;padding:0}footer{height:19mm;display:grid;grid-template-columns:35mm 40mm 46mm 1fr auto;align-items:center;gap:4mm;padding-top:3mm;border-top:.3mm solid #8f969e}footer>div:not(.sign){padding:1.5mm 2mm;border-radius:1mm;background:#f1f2f3}footer span{display:block;color:#626b75;font-size:6.5pt}footer b{display:block;font-size:9pt}.sign{text-align:center}.sign i{display:block;width:47mm;margin:4mm auto 0;border-top:.25mm solid #555}footer em{align-self:end;color:#737b84;font-size:6.5pt;font-style:normal}</style></head><body>${pages}<script>addEventListener('load',()=>setTimeout(()=>print(),700))<\/script></body></html>`);pdfWindow.document.close();
  const columnLayout=pdfWindow.document.createElement('style');columnLayout.textContent='.person{width:46mm!important}.check{width:10.5mm!important}.person b,.person small{white-space:nowrap}';pdfWindow.document.head.appendChild(columnLayout);
  preparePdfWindow(pdfWindow,`stand-roster-${new Date().toISOString().slice(0,10)}.pdf`,'landscape');
}
function enhanceStandManager(){
  ...
  const drawOptions=()=>{const q=$('#seatStudentSearch').value.trim().toLowerCase(),assignments=getStandConfig().assignments||{};filtered=directory.filter(s=>!q||`${s.name} ${s.nickname} ${s.room} ${s.number} ${s.id}`.toLowerCase().includes(q));$('#seatStudentSelect').innerHTML=filtered.map((s,i)=>{const occupied=Object.entries(assignments).find(([seat,student])=>seat!==activeSeat&&student?.id===s.id);return `<option value="${i}" ${occupied?'disabled':''}>${escapeHTML(s.name)} · ม.${s.room} เลขที่ ${s.number} · ${escapeHTML(s.nickname)}${occupied?` · มีที่นั่ง ${occupied[0]} แล้ว`:''}</option>`}).join('');};
  ...
  map.querySelectorAll('.seat').forEach(el=>{el.style.pointerEvents='auto';el.style.cursor='pointer';el.addEventListener('click',()=>openSeat(el.dataset.seat||el.querySelector('b')?.textContent.trim()))});
  $('#printAllSeatQr').onclick=printAllSeatQrs;
  $('#printAllSeatQr').textContent='↓ ดาวน์โหลด QR เป็น PDF';
  $('#printAllSeatQr').insertAdjacentHTML('beforebegin','<button type="button" id="downloadStandRoster">▤ ดาวน์โหลดใบรายชื่อ PDF</button>');
  $('#downloadStandRoster').onclick=downloadStandRosterPdf;
  $('#printSeatQr').textContent='↓ ดาวน์โหลดป้าย PDF';
  ...
  $('#saveSeatStudent').onclick=async()=>{const student=filtered[+$('#seatStudentSelect').value];if(!student)return;const button=$('#saveSeatStudent'),existing=Object.entries(getStandConfig().assignments||{}).find(([seat,saved])=>seat!==activeSeat&&saved?.id===student.id);if(existing){window.alert(`${student.name} มีที่นั่ง ${existing[0]} อยู่แล้ว ไม่สามารถเพิ่มชื่อซ้ำได้`);return}button.disabled=true;try{const result=await saveStandAssignment(activeSeat,student);if(!result.saved){button.disabled=false;window.alert(`${student.name} มีที่นั่ง ${result.duplicateSeat} อยู่แล้ว ไม่สามารถเพิ่มชื่อซ้ำได้`);return}dialog.close();renderAdminPanel('attendance');showAdminToast('ผูกข้อมูลส่วนกลางแล้ว',`${activeSeat} · ${student.name}`)}catch(error){button.disabled=false;window.alert(firebaseWriteErrorMessage(error))}};
  $('#clearSeatStudent').onclick=async()=>{const button=$('#clearSeatStudent');button.disabled=true;try{await saveStandAssignment(activeSeat,null);dialog.close();renderAdminPanel('attendance')}catch(error){button.disabled=false;window.alert(firebaseWriteErrorMessage(error))}};
  $('#printSeatQr').onclick=()=>{const s=filtered[+$('#seatStudentSelect').value],img=$('#seatQr img')?.src||$('#seatQr canvas')?.toDataURL();if(!img)return;const w=open('','_blank','width=500,height=650');if(!w){alert('เบราว์เซอร์บล็อกหน้าต่างดาวน์โหลด PDF กรุณาอนุญาต Pop-up');return}w.document.write(`<title>QR ${activeSeat}</title><style>@page{size:A4 portrait;margin:15mm}body{text-align:center;font-family:sans-serif;padding:35px}img{width:280px}h1{font-size:60px;margin:10px}p{font-size:18px}</style><img src="${img}"><h1>${activeSeat}</h1><p>${escapeHTML(s?.name||'ยังไม่ระบุนักเรียน')}<br>${s?`ม.${s.room} เลขที่ ${s.number} · ${escapeHTML(s.nickname)}`:''}</p>`);w.document.close();preparePdfWindow(w,`seat-${activeSeat}-qr.pdf`,'portrait');setTimeout(()=>w.print(),400)};
}

const ACTIVITY_DATA_KEY='phanuang-activity-attendance';
const ACTIVITY_GROUPS=['บาสเกตบอล','วอลเลย์บอล','แบดมินตัน','เปตอง','เซปักตะกร้อ','กรีฑา','ฟุตบอล','กีฬาลาน','ฟุตซอล','ดรัมเมเยอร์','เชียร์ลีดเดอร์','ขบวนพาเหรด','สวัสดิการ/พยาบาล','คัตเอาท์'];
const normalizeActivityGroup=group=>['กระโดดไกล','ขว้างจักร'].includes(group)?'กีฬาลาน':group;
function normalizeActivityData(value){
  const rosters=value?.rosters&&typeof value.rosters==='object'?{...value.rosters}:{},legacyFieldGroups=['กระโดดไกล','ขว้างจักร'];
  const fieldRoster=[...(rosters['กีฬาลาน']||[]),...legacyFieldGroups.flatMap(group=>rosters[group]||[])];
  if(fieldRoster.length)rosters['กีฬาลาน']=[...new Map(fieldRoster.map(student=>[student.id,student])).values()];
  legacyFieldGroups.forEach(group=>delete rosters[group]);
  const sessions=(Array.isArray(value?.sessions)?value.sessions:[]).map(session=>legacyFieldGroups.includes(session.group)?{...session,group:'กีฬาลาน'}:session);
  return {rosters,sessions};
}
function getActivityData(){try{return normalizeActivityData(JSON.parse(localStorage.getItem(ACTIVITY_DATA_KEY)||'{}'))}catch(_){return normalizeActivityData({})}}
async function updateActivityData(change){
  if(!firebaseReady||!firebaseSyncRoot)throw Object.assign(new Error('Firebase is not ready'),{code:'firebase/not-ready'});
  if(!firebaseAuthUser)throw Object.assign(new Error('Admin authentication is required'),{code:'auth/admin-required'});
  const ref=firebaseSyncRoot.child(firebaseKey(ACTIVITY_DATA_KEY));
  const result=await ref.transaction(cloudRecord=>{let current;try{current=normalizeActivityData(JSON.parse(cloudRecord?.value||'{}'))}catch(_){current=normalizeActivityData({})}const next=normalizeActivityData(change(current)||current);return {value:JSON.stringify(next),updatedAt:firebase.database.ServerValue.TIMESTAMP}},undefined,false);
  if(!result.committed)throw new Error('บันทึกข้อมูลไม่สำเร็จ');
  const serialized=result.snapshot.val()?.value||'{"rosters":{},"sessions":[]}';
  firebaseManualWriteKeys.add(ACTIVITY_DATA_KEY);try{localStorage.setItem(ACTIVITY_DATA_KEY,serialized)}finally{firebaseManualWriteKeys.delete(ACTIVITY_DATA_KEY)}
  return normalizeActivityData(JSON.parse(serialized));
}
function openActivityStudentPicker({directory,roster,group,data,host}){
  document.querySelector('.activity-student-picker')?.remove();
  const selected=new Set(roster.map(s=>s.id)),rooms=Object.keys(CLASSROOM_DATABASE).sort(electionRoomSort);let room=rooms[0],query='';
  const dialog=document.createElement('dialog');dialog.className='activity-student-picker';dialog.innerHTML=`<header><div><small>STUDENT DIRECTORY</small><h3>เลือกรายชื่อ ${escapeHTML(group)}</h3><p>เลือกได้หลายคนจากหลายห้อง แล้วบันทึกครั้งเดียว</p></div><button type="button" data-picker-close>×</button></header><div class="student-picker-tools"><label><span>⌕</span><input id="multiStudentSearch" placeholder="ค้นหาชื่อ ชื่อเล่น หรือรหัสนักเรียน"></label><div class="student-selected-count"><b id="multiSelectedCount">${selected.size}</b><span>คนที่เลือก</span></div></div><nav class="student-room-tabs">${rooms.map(r=>`<button type="button" data-picker-room="${r}" class="${r===room?'active':''}">ม.${r}<small>${CLASSROOM_DATABASE[r].students.length} คน</small></button>`).join('')}</nav><div class="student-picker-actions"><span id="pickerRoomLabel"></span><div><button type="button" id="selectVisibleStudents">✓ เลือกทั้งห้องนี้</button><button type="button" id="clearVisibleStudents">ยกเลิกห้องนี้</button></div></div><div class="student-card-grid" id="studentCardGrid"></div><footer><button type="button" id="cancelStudentPicker">ยกเลิก</button><button type="button" class="primary" id="saveSelectedStudents">เพิ่มรายชื่อที่เลือก <b>(${selected.size})</b></button></footer>`;document.body.appendChild(dialog);
  const visible=()=>directory.filter(s=>(query?`${s.id} ${s.name} ${s.nickname} ${s.room}`.toLowerCase().includes(query):s.room===room));
  const updateCount=()=>{$('#multiSelectedCount').textContent=selected.size;$('#saveSelectedStudents b').textContent=`(${selected.size})`};
  const draw=()=>{const students=visible();$('#pickerRoomLabel').textContent=query?`ผลการค้นหา ${students.length} คน`:`ห้อง ม.${room} · ${students.length} คน`;$('#studentCardGrid').innerHTML=students.map(s=>`<label class="${selected.has(s.id)?'selected':''}"><input type="checkbox" value="${s.id}" ${selected.has(s.id)?'checked':''}><i>${s.number}</i><span><b>${escapeHTML(s.name)}</b><small>${s.nickname&&s.nickname!=='—'?escapeHTML(s.nickname):'ไม่มีชื่อเล่น'} · ${s.id}</small></span><em>✓</em></label>`).join('')||'<div class="student-picker-empty">ไม่พบนักเรียนที่ค้นหา</div>';document.querySelectorAll('#studentCardGrid input').forEach(input=>input.onchange=()=>{input.checked?selected.add(input.value):selected.delete(input.value);input.closest('label').classList.toggle('selected',input.checked);updateCount()})};
  document.querySelectorAll('[data-picker-room]').forEach(button=>button.onclick=()=>{room=button.dataset.pickerRoom;query='';$('#multiStudentSearch').value='';document.querySelectorAll('[data-picker-room]').forEach(b=>b.classList.toggle('active',b===button));draw()});
  $('#multiStudentSearch').oninput=e=>{query=e.target.value.trim().toLowerCase();draw()};
  $('#selectVisibleStudents').onclick=()=>{visible().forEach(s=>selected.add(s.id));draw();updateCount()};
  $('#clearVisibleStudents').onclick=()=>{visible().forEach(s=>selected.delete(s.id));draw();updateCount()};
  const close=()=>dialog.close();$('[data-picker-close]').onclick=close;$('#cancelStudentPicker').onclick=close;
  $('#saveSelectedStudents').innerHTML=`บันทึกรายชื่อส่วนกลาง <b>(${selected.size})</b>`;
  $('#saveSelectedStudents').onclick=async()=>{const button=$('#saveSelectedStudents'),nextRoster=directory.filter(s=>selected.has(s.id));button.disabled=true;button.firstChild.textContent='กำลังบันทึก… ';try{await updateActivityData(current=>{current.rosters[group]=nextRoster;return current});close();renderActivityAttendance(host);showAdminToast('บันทึกรายชื่อส่วนกลางแล้ว',`${group} จำนวน ${selected.size} คน · ทุกเครื่องจะอัปเดตอัตโนมัติ`)}catch(error){button.disabled=false;button.firstChild.textContent='บันทึกรายชื่อส่วนกลาง ';window.alert(adminSaveErrorMessage(error))}};
  dialog.addEventListener('close',()=>dialog.remove(),{once:true});draw();dialog.showModal();setTimeout(()=>$('#multiStudentSearch').focus(),50);
}
const ACTIVITY_HEAD_ROLE_PATTERNS={
  'บาสเกตบอล':[/เฮดบาส/], 'วอลเลย์บอล':[/เฮดวอลเลย์บอล/], 'แบดมินตัน':[/เฮดแบดมินตัน/],
  'เปตอง':[/เฮดเปตอง/], 'เซปักตะกร้อ':[/เฮดเซปักตะกร้อ/], 'ฟุตบอล':[/เฮดฟุตบอล/],
  'ฟุตซอล':[/เฮดฟุตซอล/], 'กรีฑา':[/เฮดกรีฑา/], 'กีฬาลาน':[/เฮดกีฬาประเภทลาน/,/เฮดกีฬาลาน/,/เฮดกรีฑา/],
  'ดรัมเมเยอร์':[/เฮดหลีด.*ดรัม/,/เฮดดรัม/],
  'เชียร์ลีดเดอร์':[/เฮดหลีด.*ดรัม/,/เฮดเชียร์/], 'ขบวนพาเหรด':[/เฮดขบวน/],
  'สวัสดิการ/พยาบาล':[/สวัสดิการ.*พยาบาล/,/เฮดสวัสดิการ/], 'คัตเอาท์':[/เฮดคัตเอาท์/]
};
function activityHeadsForGroup(group,members){
  const patterns=ACTIVITY_HEAD_ROLE_PATTERNS[group]||[new RegExp(`เฮด.*${String(group).replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}`)];
  return members.filter(member=>member.published!==false&&member.type!=='teacher'&&patterns.some(pattern=>pattern.test(String(member.role||''))));
}
async function downloadActivityRosterPdf(group,roster){
  if(!roster.length){alert(`ยังไม่มีรายชื่อใน ${group} กรุณาเพิ่มรายชื่อก่อนดาวน์โหลด PDF`);return}
  const pdfWindow=window.open('','_blank','width=1280,height=850');if(!pdfWindow){alert('เบราว์เซอร์บล็อกหน้าต่างดาวน์โหลด PDF กรุณาอนุญาต Pop-up');return}
  pdfWindow.document.write('<!doctype html><title>กำลังสร้างใบรายชื่อ PDF…</title><style>body{display:grid;min-height:100vh;place-items:center;margin:0;font-family:sans-serif;color:#273243}p{text-align:center}</style><p><b>กำลังจัดทำใบรายชื่อ</b><br>โปรดรอสักครู่…</p>');pdfWindow.document.close();
  const members=await getCommitteeMembers(),heads=activityHeadsForGroup(group,members),headNames=heads.length?heads.map(member=>`${member.name}${member.nickname?` (${member.nickname})`:''}`).join(', '):'ยังไม่ได้ระบุเฮดในหน้าสมาชิก';
  const checks=Array.from({length:6},()=>'<th class="check"></th>').join(''),emptyChecks=Array.from({length:6},()=>'<td class="check"></td>').join('');
  const row=(student,index)=>`<tr><td class="number">${student?index+1:''}</td><td class="person">${student?`<b>${escapeHTML(student.name||'')}</b>${student.nickname&&student.nickname!=='—'?`<small> / ${escapeHTML(student.nickname)}</small>`:''}`:''}</td><td class="room">${student?`ม.${escapeHTML(student.room||'—')}`:''}</td>${emptyChecks}</tr>`;
  const table=(start)=>`<table><thead><tr><th class="number">ที่</th><th class="person">ชื่อจริง / ชื่อเล่น</th><th class="room">ชั้น / ห้อง</th>${checks}</tr></thead><tbody>${Array.from({length:25},(_,offset)=>row(roster[start+offset],start+offset)).join('')}</tbody></table>`;
  const pages=Math.max(1,Math.ceil(roster.length/50)),documentPages=Array.from({length:pages},(_,pageIndex)=>{const start=pageIndex*50;return `<section class="roster-page"><header><div class="brand"><div class="crest"><b>PR</b><small>พันเรือง</small></div><div><small>โรงเรียน · คณะสีพันเรือง</small><h1>ใบรายชื่อ ${escapeHTML(group)}</h1><p>ปีการศึกษา 2569</p></div></div><div class="head-box"><span>เฮดผู้รับผิดชอบ</span><b>${escapeHTML(headNames)}</b></div></header><div class="tables">${table(start)}${table(start+25)}</div><footer><div><span>สรุปจำนวนนักกีฬา / สมาชิกฝ่าย</span><b>${roster.length} คน</b></div><div><span>สรุปจำนวนเฮด</span><b>${heads.length} คน</b></div><div class="sign"><span>ลงชื่อผู้รับผิดชอบ</span><i></i><small>( ${escapeHTML(headNames)} )</small></div><em>หน้า ${pageIndex+1} / ${pages}</em></footer></section>`}).join('');
  pdfWindow.document.open();pdfWindow.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>ใบรายชื่อ ${escapeHTML(group)}</title><link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;500;600;700&display=swap" rel="stylesheet"><style>@page{size:A4 landscape;margin:0}*{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}html,body{margin:0;background:#dfe3e8;color:#17202b;font-family:Sarabun,Tahoma,sans-serif}.roster-page{width:297mm;height:210mm;margin:0 auto;padding:8mm 9mm 7mm;background:#fff;break-after:page;page-break-after:always;overflow:hidden}.roster-page:last-child{break-after:auto;page-break-after:auto}.roster-page>header{height:29mm;display:flex;align-items:center;justify-content:space-between;gap:8mm;padding-bottom:3mm;border-bottom:1.2mm solid #d6aa23}.brand{display:flex;align-items:center;gap:4mm}.crest{width:22mm;height:22mm;display:flex;align-items:center;justify-content:center;flex-direction:column;border:1mm double #bd8e08;border-radius:50%;background:#fff9df;color:#8d6600}.crest b{font-size:15pt;line-height:1}.crest small{margin-top:1mm;font-size:6pt;font-weight:700}.brand>div>small{color:#8a6a16;font-size:7pt;letter-spacing:.7pt}.brand h1{margin:.5mm 0;font-size:18pt}.brand p{margin:0;color:#59616c;font-size:8pt}.head-box{width:90mm;min-height:19mm;padding:3mm 4mm;border:1px solid #c9cdd2;border-radius:2mm;background:#f8f9fa}.head-box span{display:block;color:#68717c;font-size:7pt}.head-box b{display:block;margin-top:1mm;font-size:10pt;line-height:1.35}.tables{height:143mm;display:grid;grid-template-columns:1fr 1fr;gap:5mm;padding-top:3mm}table{width:100%;height:100%;border-collapse:collapse;table-layout:fixed;font-size:6.7pt}th,td{height:5mm;padding:.6mm 1mm;border:.25mm solid #555;line-height:1.05}th{background:#ece8d8;text-align:center;font-weight:700}.number{width:8mm;text-align:center}.person{width:auto;text-align:left}.person b{font-weight:600}.person small{font-size:6pt}.room{width:17mm;text-align:center}.check{width:7mm;padding:0}footer{height:24mm;display:grid;grid-template-columns:40mm 34mm 1fr auto;align-items:center;gap:6mm;padding-top:3mm;border-top:.3mm solid #aeb4ba}footer>div:not(.sign){padding:2mm 3mm;border-radius:1.5mm;background:#f1f2f3}footer span{display:block;color:#626b75;font-size:7pt}footer b{display:block;margin-top:.5mm;font-size:11pt}.sign{text-align:center}.sign i{display:block;width:60mm;margin:4mm auto 1mm;border-top:.25mm solid #555}.sign small{font-size:7pt}footer em{align-self:end;color:#7c848d;font-size:7pt;font-style:normal}</style></head><body>${documentPages}<script>addEventListener('load',()=>setTimeout(()=>print(),700))<\/script></body></html>`);pdfWindow.document.close();
  preparePdfWindow(pdfWindow,`roster-${group}-${new Date().toISOString().slice(0,10)}.pdf`,'landscape');
}
function printActivityHistoryReport(group,data,directory){
  const sessions=data.sessions.filter(item=>item.group===group);if(!sessions.length){alert(`ยังไม่มีประวัติของ ${group}`);return}
  const byId=new Map(directory.map(student=>[student.id,student])),personRow=(id,index,status)=>{const s=byId.get(id);return `<tr><td>${index+1}</td><td><b>${escapeHTML(s?.name||`รหัส ${id}`)}</b>${s?.nickname&&s.nickname!=='—'?`<small>${escapeHTML(s.nickname)}</small>`:''}</td><td>${s?`ม.${s.room}`:'—'}</td><td>${s?.number||'—'}</td><td>${id}</td><td><span class="status ${status}">${status==='present'?'มา':'ขาด'}</span></td></tr>`};
  const rounds=sessions.map((item,index)=>{const savedRoster=Array.isArray(item.roster)?item.roster:[],rosterIds=savedRoster.length?savedRoster:[...new Set([...(data.rosters[group]||[]).map(s=>s.id),...(item.present||[])])],present=rosterIds.filter(id=>(item.present||[]).includes(id)),absent=rosterIds.filter(id=>!(item.present||[]).includes(id));return `<section class="round"><header><div><small>รอบที่ ${sessions.length-index}</small><h2>${new Date(item.date).toLocaleDateString('th-TH',{dateStyle:'long'})}</h2></div><div class="stats"><span class="present">มา <b>${present.length}</b></span><span class="absent">ขาด <b>${absent.length}</b></span><span>ทั้งหมด <b>${rosterIds.length}</b></span></div></header><table><thead><tr><th>#</th><th>ชื่อ–นามสกุล / ชื่อเล่น</th><th>ชั้น</th><th>เลขที่</th><th>รหัสนักเรียน</th><th>สถานะ</th></tr></thead><tbody>${present.map((id,i)=>personRow(id,i,'present')).join('')}${absent.map((id,i)=>personRow(id,present.length+i,'absent')).join('')}</tbody></table></section>`}).join('');
  const printWindow=window.open('','_blank','width=1100,height=800');if(!printWindow){alert('เบราว์เซอร์บล็อกหน้าต่างพิมพ์ กรุณาอนุญาต Pop-up');return}
  printWindow.document.open();printWindow.document.write(`<!doctype html><html lang="th"><head><meta charset="UTF-8"><title>ประวัติเช็คชื่อ ${escapeHTML(group)}</title><link href="https://fonts.googleapis.com/css2?family=Kanit:wght@400;500;600;700&display=swap" rel="stylesheet"><style>@page{size:A4 portrait;margin:12mm}*{box-sizing:border-box}body{margin:0;color:#20242c;font-family:Kanit,sans-serif}.report-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:8mm;padding-bottom:5mm;border-bottom:3px solid #e6b92e}.report-head small{color:#9a7410;letter-spacing:2px}.report-head h1{margin:1mm 0;font-size:22pt}.report-head p{margin:0;color:#687080}.mark{width:20mm;height:20mm;display:grid;place-items:center;border:2px solid #e0b12a;border-radius:50%;color:#a77c00;font-size:18pt;font-weight:700}.round{margin-bottom:9mm;break-inside:avoid-page}.round>header{display:flex;align-items:end;justify-content:space-between;margin-bottom:3mm;padding:3mm 4mm;border-left:4px solid #e2b329;background:#f7f4e9}.round header small{color:#8c6c14}.round h2{margin:0;font-size:15pt}.stats{display:flex;gap:2mm}.stats span{padding:2mm 3mm;border-radius:2mm;background:#e9edf1;font-size:9pt}.stats .present{background:#dcefe7;color:#176449}.stats .absent{background:#f5e0e4;color:#963648}table{width:100%;border-collapse:collapse;font-size:9pt}th{padding:2.2mm;background:#263143;color:#fff;text-align:left}td{padding:2mm;border-bottom:1px solid #dce0e5}td:first-child{width:9mm;text-align:center}td small{display:block;color:#777}.status{display:inline-block;min-width:13mm;padding:1mm 2mm;border-radius:99px;text-align:center;font-weight:600}.status.present{background:#d8eee5;color:#176047}.status.absent{background:#f3dce1;color:#963448}.report-foot{margin-top:6mm;padding-top:3mm;border-top:1px solid #ccd1d8;color:#777;font-size:8pt;text-align:right}@media print{.round{break-inside:avoid}thead{display:table-header-group}}</style></head><body><header class="report-head"><div><small>PHUNRUEANG · TEAM ATTENDANCE</small><h1>ประวัติการเช็คชื่อ ${escapeHTML(group)}</h1><p>ทั้งหมด ${sessions.length} รอบ · แยกรายชื่อผู้มาและผู้ขาดรายรอบ</p></div><div class="mark">PR</div></header>${rounds}<footer class="report-foot">พิมพ์จากระบบคณะสีพันเรือง · ${new Date().toLocaleString('th-TH',{dateStyle:'long',timeStyle:'short'})}</footer><script>addEventListener('load',()=>setTimeout(()=>print(),350))<\/script></body></html>`);printWindow.document.close();
  preparePdfWindow(printWindow,`activity-attendance-${group}-${Date.now()}.pdf`,'portrait');
}
function renderActivityAttendance(host){const data=getActivityData(),directory=attendanceStudentDirectory();let group=normalizeActivityGroup(localStorage.getItem('phanuang-active-group')||ACTIVITY_GROUPS[0]),roster=data.rosters[group]||[],session=data.sessions.filter(s=>s.group===group)[0],today=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);
  localStorage.setItem('phanuang-active-group',group);
  activityAttendanceDraftDirty=false;
  $('#admin')?.classList.add('activity-fast-mode');
  host.innerHTML=`<div class="admin-panel activity-panel"><header class="activity-simple-head"><div><small>เช็คชื่อการซ้อม</small><h3>${escapeHTML(group)}</h3><p>เลือกกลุ่ม จัดรายชื่อ แล้วแตะชื่อคนที่มาซ้อม</p></div><div class="activity-live-summary"><b id="activityPresentCount">${session?.present?.length||0}</b><span>มา จาก ${roster.length} คน</span></div></header><section class="activity-step activity-group-step"><span class="activity-step-number">1</span><label><b>เลือกกลุ่มที่ต้องการเช็ค</b><select id="activityGroupSelect">${ACTIVITY_GROUPS.map(g=>`<option ${g===group?'selected':''}>${g}</option>`).join('')}</select></label><button type="button" id="printActivity">▣ พิมพ์ประวัติ</button></section><section class="activity-step roster-builder"><span class="activity-step-number">2</span><div><h4>จัดรายชื่อผู้เข้าร่วม</h4><small>ค้นหาด้วยรหัสนักเรียน ชื่อ ห้อง หรือชื่อเล่น</small></div><div class="roster-add"><input id="activitySearch" list="activityStudents" placeholder="พิมพ์เพื่อค้นหานักเรียน"><datalist id="activityStudents">${directory.map(s=>`<option value="${s.id}">${escapeHTML(s.name)} · ม.${s.room} เลขที่ ${s.number} · ${escapeHTML(s.nickname)}</option>`).join('')}</datalist><button class="primary" id="addActivityStudent">＋ เพิ่ม</button></div></section><section class="activity-check-head"><div><span class="activity-step-number">3</span><div><h4>แตะรายชื่อคนที่มาซ้อม</h4><small id="activityCheckHint">สีเขียว = มา · สีเทา = ยังไม่มา</small></div></div><div><button type="button" id="markAllActivity">เลือกมาทั้งหมด</button><button type="button" id="clearAllActivity">ล้างทั้งหมด</button></div></section><div class="activity-list" id="activityPrintReport">${roster.length?roster.map((s,index)=>`<label class="${session?.present?.includes(s.id)?'is-present':''}"><input type="checkbox" data-student="${s.id}" ${session?.present?.includes(s.id)?'checked':''}><i>${index+1}</i><span><b>${escapeHTML(s.name)}</b><small>ม.${s.room} · เลขที่ ${s.number}${s.nickname&&s.nickname!=='—'?` · ${escapeHTML(s.nickname)}`:''}</small></span><em>${session?.present?.includes(s.id)?'✓ มา':'ยังไม่มา'}</em><button type="button" data-remove="${s.id}" title="ลบจากรายชื่อ">×</button></label>`).join(''):'<div class="activity-empty"><b>ยังไม่มีรายชื่อในกลุ่มนี้</b><span>ค้นหานักเรียนด้านบนแล้วกด “เพิ่ม”</span></div>'}</div><footer class="activity-savebar"><label><span>วันที่ซ้อม</span><input id="activityDate" type="date" value="${session?.date||today}"></label><div><button type="button" id="newActivitySession">ล้างเพื่อเริ่มรอบใหม่</button><button class="primary" id="saveActivitySession">บันทึกผลเช็คชื่อ <b>→</b></button></div></footer><details class="activity-history"><summary>ดูประวัติ ${escapeHTML(group)} (${data.sessions.filter(s=>s.group===group).length} รอบ)</summary>${data.sessions.filter(s=>s.group===group).map(s=>`<div><b>${new Date(s.date).toLocaleDateString('th-TH',{dateStyle:'long'})}</b><span>มา ${s.present.length} / ${s.roster.length} · ขาด ${s.roster.length-s.present.length}</span></div>`).join('')||'<p>ยังไม่มีประวัติ</p>'}</details></div>`;
  document.querySelector('.activity-group-step').insertAdjacentHTML('beforebegin',`<section class="activity-department-picker"><header><span class="activity-step-number">1</span><div><b>เลือกชนิดกีฬาหรือฝ่าย</b><small>เลือกเพียงครั้งเดียวจากรายการด้านล่าง</small></div></header><nav class="activity-department-board" aria-label="ฝ่ายและชนิดกีฬา">${ACTIVITY_GROUPS.map(name=>`<button type="button" data-activity-group="${escapeAttribute(name)}" class="${name===group?'active':''}"><b>${escapeHTML(name)}</b><span>${(data.rosters[name]||[]).length} คน · ${data.sessions.filter(item=>item.group===name).length} รอบ</span></button>`).join('')}</nav></section>`);
  document.querySelector('.activity-group-step').remove();
  document.querySelector('.activity-simple-head').insertAdjacentHTML('beforeend','<button type="button" id="printActivity">▣ พิมพ์ประวัติ</button>');
  $('#printActivity').textContent='↓ ดาวน์โหลดประวัติ PDF';
  $('#printActivity').insertAdjacentHTML('beforebegin','<button type="button" class="activity-roster-pdf" id="downloadActivityRoster">▤ ดาวน์โหลดใบรายชื่อ PDF</button>');
  document.querySelectorAll('[data-activity-group]').forEach(button=>button.onclick=()=>{localStorage.setItem('phanuang-active-group',button.dataset.activityGroup);renderActivityAttendance(host)});
  const rosterAdd=document.querySelector('.activity-panel .roster-add');rosterAdd.innerHTML=`<button class="activity-open-picker" type="button" id="openActivityPicker"><span>♙</span><div><b>เลือกรายชื่อนักเรียน</b><small>เลือกพร้อมกันได้หลายคน หรือเลือกทั้งห้อง</small></div><i>＋ เปิดรายชื่อ</i></button>`;
  $('#openActivityPicker').onclick=()=>openActivityStudentPicker({directory,roster,group,data,host});
  const groupHistory=data.sessions.filter(item=>item.group===group),historyBox=document.querySelector('.activity-panel .activity-history'),studentById=new Map(directory.map(student=>[student.id,student]));
  const historyPerson=(id,isPresent)=>{const student=studentById.get(id);return `<li><i>${isPresent?'✓':'×'}</i><span><b>${escapeHTML(student?.name||`รหัส ${id}`)}</b><small>${student?`ม.${student.room} · เลขที่ ${student.number}${student.nickname&&student.nickname!=='—'?` · ${escapeHTML(student.nickname)}`:''}`:`รหัสนักเรียน ${id}`}</small></span></li>`};
  historyBox.innerHTML=`<summary>ดูประวัติ ${escapeHTML(group)} <b>${groupHistory.length} รอบ</b></summary><div class="activity-round-list">${groupHistory.map((item,index)=>{const rosterIds=Array.isArray(item.roster)?item.roster:[],presentIds=rosterIds.filter(id=>item.present.includes(id)),absentIds=rosterIds.filter(id=>!item.present.includes(id));return `<details class="activity-history-round" ${index===0?'open':''}><summary><div><small>${index===0?'รอบล่าสุด':'รอบก่อนหน้า'}</small><b>${new Date(item.date).toLocaleDateString('th-TH',{dateStyle:'long'})}</b></div><span class="round-present">มา <b>${presentIds.length}</b></span><span class="round-absent">ขาด <b>${absentIds.length}</b></span>${index>0?`<button type="button" class="delete-activity-round" data-delete-activity="${escapeAttribute(item.id)}">ลบรอบนี้</button>`:''}<i>⌄</i></summary><div class="activity-round-detail"><section class="round-name-list present"><header><span>✓</span><div><b>มาซ้อม</b><small>${presentIds.length} คน</small></div></header><ul>${presentIds.map(id=>historyPerson(id,true)).join('')||'<li class="no-names">ไม่มีผู้มาซ้อม</li>'}</ul></section><section class="round-name-list absent"><header><span>×</span><div><b>ขาดซ้อม</b><small>${absentIds.length} คน</small></div></header><ul>${absentIds.map(id=>historyPerson(id,false)).join('')||'<li class="no-names">ไม่มีผู้ขาดซ้อม</li>'}</ul></section></div></details>`}).join('')||'<div class="activity-no-history">ยังไม่มีประวัติการเช็คชื่อ</div>'}</div>`;
  historyBox.querySelectorAll('.activity-history-round').forEach((round,index)=>{const item=groupHistory[index],detail=round.querySelector('.activity-round-detail');round.querySelectorAll('[data-delete-activity]').forEach(button=>button.remove());if(!detail||!item)return;const actions=document.createElement('div');actions.className='activity-history-actions';actions.innerHTML=`<span>จัดการประวัติรอบนี้</span><button type="button" class="delete-activity-round" data-delete-activity="${escapeAttribute(item.id)}" data-latest="${index===0}">${index===0?'⌫ ลบประวัติล่าสุด':'⌫ ลบประวัตินี้'}</button>`;detail.insertBefore(actions,detail.firstChild)});
  document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=async e=>{e.preventDefault();b.disabled=true;try{await updateActivityData(current=>{current.rosters[group]=(current.rosters[group]||[]).filter(s=>s.id!==b.dataset.remove);return current});renderActivityAttendance(host)}catch(error){b.disabled=false;window.alert(adminSaveErrorMessage(error))}});
  document.querySelectorAll('[data-delete-activity]').forEach(button=>button.onclick=async event=>{event.preventDefault();event.stopPropagation();const latest=button.dataset.latest==='true',message=latest?'ลบประวัติล่าสุดของฝ่ายนี้หรือไม่? รายชื่อประจำฝ่ายจะยังอยู่ แต่ผลมา/ขาดของรอบนี้จะถูกลบ':'ลบเฉพาะประวัติรอบที่เลือกหรือไม่? รายชื่อประจำฝ่ายจะไม่ถูกลบ';if(!confirm(message))return;button.disabled=true;try{await updateActivityData(current=>{current.sessions=current.sessions.filter(item=>item.id!==button.dataset.deleteActivity);return current});renderActivityAttendance(host);showAdminToast('ลบประวัติแล้ว',latest?'ลบผลรอบล่าสุดแล้ว · รายชื่อประจำฝ่ายยังอยู่':'ลบเฉพาะรอบที่เลือกแล้ว')}catch(error){button.disabled=false;window.alert(adminSaveErrorMessage(error))}});
  const syncChecks=()=>{const checks=[...document.querySelectorAll('[data-student]')];checks.forEach(x=>{const row=x.closest('label');row.classList.toggle('is-present',x.checked);row.querySelector('em').textContent=x.checked?'✓ มา':'ยังไม่มา'});$('#activityPresentCount').textContent=checks.filter(x=>x.checked).length};
  document.querySelectorAll('[data-student]').forEach(x=>x.onchange=()=>{activityAttendanceDraftDirty=true;syncChecks()});
  $('#activityDate').onchange=()=>{activityAttendanceDraftDirty=true};
  $('#markAllActivity').onclick=()=>{activityAttendanceDraftDirty=true;document.querySelectorAll('[data-student]').forEach(x=>x.checked=true);syncChecks()};
  $('#clearAllActivity').onclick=()=>{activityAttendanceDraftDirty=true;document.querySelectorAll('[data-student]').forEach(x=>x.checked=false);syncChecks()};
  $('#newActivitySession').onclick=()=>{activityAttendanceDraftDirty=true;session=null;document.querySelectorAll('[data-student]').forEach(x=>x.checked=false);$('#activityDate').value=today;syncChecks()};
  $('#saveActivitySession').onclick=async()=>{const button=$('#saveActivitySession'),date=$('#activityDate').value,present=[...document.querySelectorAll('[data-student]:checked')].map(x=>x.dataset.student),record={id:`activity-${Date.now()}`,group,date,present,roster:roster.map(s=>s.id),createdAt:new Date().toISOString()};button.disabled=true;button.innerHTML='กำลังบันทึก…';try{await updateActivityData(current=>{current.sessions=current.sessions.filter(s=>!(s.group===group&&s.date===date));current.sessions.unshift(record);return current});renderActivityAttendance(host);showAdminToast('บันทึกผลเช็คชื่อส่วนกลางแล้ว',`${group} มา ${present.length}/${roster.length} คน`)}catch(error){button.disabled=false;button.innerHTML='บันทึกผลเช็คชื่อ <b>→</b>';window.alert(adminSaveErrorMessage(error))}};
  $('#downloadActivityRoster').onclick=()=>downloadActivityRosterPdf(group,roster);
  $('#printActivity').onclick=()=>printActivityHistoryReport(group,data,directory);
}
...
const SPORT_COLORS = ['สีเหลือง','สีแสด','สีน้ำเงิน','สีชมพู','สีม่วง','สีเขียว'];
const SPORT_COLOR_ALIASES = {'สีแดง':'สีแสด','สีส้ม':'สีแสด','สีฟ้า':'สีน้ำเงิน','เหลือง':'สีเหลือง','แดง':'สีแสด','ส้ม':'สีแสด','แสด':'สีแสด','ฟ้า':'สีน้ำเงิน','น้ำเงิน':'สีน้ำเงิน','ชมพู':'สีชมพู','ม่วง':'สีม่วง','เขียว':'สีเขียว'};
function normalizeSportColor(value,fallback=''){const normalized=SPORT_COLOR_ALIASES[value]||value;return SPORT_COLORS.includes(normalized)?normalized:fallback;}
const defaultSportsData = [
  {id:'football',date:'2026-08-01',name:'ฟุตบอล',icon:'⚽',venue:'สนามฟุตบอล',status:'live',yellowRank:0,matches:[{round:'รอบรองชนะเลิศ',a:'สีเหลือง',b:'สีแสด',scoreA:'2',scoreB:'1',winner:'สีเหลือง'},{round:'รอบรองชนะเลิศ',a:'สีน้ำเงิน',b:'สีเขียว',scoreA:'0',scoreB:'1',winner:'สีเขียว'},{round:'รอบชิงชนะเลิศ',a:'สีเหลือง',b:'สีเขียว',scoreA:'-',scoreB:'-',winner:''}]},
  {id:'badminton',date:'2026-08-01',name:'แบดมินตัน',icon:'🏸',venue:'โรงยิม',status:'finished',yellowRank:1,matches:[{round:'รอบรองชนะเลิศ',a:'สีเหลือง',b:'สีม่วง',scoreA:'2',scoreB:'0',winner:'สีเหลือง'},{round:'รอบชิงชนะเลิศ',a:'สีเหลือง',b:'สีแสด',scoreA:'2',scoreB:'1',winner:'สีเหลือง'}]},
  {id:'basketball',date:'2026-08-02',name:'บาสเกตบอล',icon:'🏀',venue:'สนาม 2',status:'upcoming',yellowRank:0,matches:[{round:'รอบแรก',a:'สีเหลือง',b:'สีน้ำเงิน',scoreA:'-',scoreB:'-',winner:''}]},
...
function saveSportsData(data){ data.forEach(s=>s.matches.forEach(m=>{ const winner=String(m.winner||'').replace('ชนะ: ','');m.a=normalizeSportColor(m.a,'สีเหลือง');m.b=normalizeSportColor(m.b,'สีแสด');m.winner=normalizeSportColor(winner,'');if(m.matchType==='bye'){m.byeTeam=normalizeSportColor(m.byeTeam||m.a,'สีเหลือง');m.a=m.byeTeam;m.winner=m.byeTeam;m.scoreA='-';m.scoreB='-';}else{delete m.byeTeam;delete m.matchType;} })); localStorage.setItem(SPORTS_KEY,JSON.stringify(data)); }
...
function legacyRenderSportsAdmin(host){
  ...
  const bind=()=>{ document.querySelectorAll('.sport-editor').forEach(section=>{const si=+section.dataset.si; section.querySelectorAll('[data-field]').forEach(el=>el.addEventListener('input',()=>{data[si][el.dataset.field]=el.type==='number'?+el.value:el.value;$('#sportSaveState').textContent='มีการแก้ไขที่ยังไม่บันทึก';}));section.querySelectorAll('[data-match]').forEach(el=>el.addEventListener('input',()=>{data[si].matches[+el.dataset.match][el.dataset.mf]=el.value;}));section.querySelector('.add-match').addEventListener('click',()=>{data[si].matches.push({round:'รอบแรก',a:'สีเหลือง',b:'สีแสด',scoreA:'-',scoreB:'-',winner:''});draw();});section.querySelectorAll('[data-remove-match]').forEach(b=>b.addEventListener('click',()=>{data[si].matches.splice(+b.dataset.removeMatch,1);draw();}));section.querySelector('[data-delete]').addEventListener('click',()=>{if(confirm(`ลบกีฬา ${data[si].name} ใช่หรือไม่?`)){data.splice(si,1);draw();}});});};
...
function normalizeSportsData(){
  return getSportsData().map((sport,index)=>({
    ...
    matches:(sport.matches||[]).map(match=>{const byeTeam=normalizeSportColor(match.byeTeam||match.a,'สีเหลือง'),isBye=match.matchType==='bye';return {...match,matchType:isBye?'bye':'normal',byeTeam:isBye?byeTeam:'',a:isBye?byeTeam:normalizeSportColor(match.a,'สีเหลือง'),b:normalizeSportColor(match.b,'สีแสด'),winner:isBye?byeTeam:normalizeSportColor(String(match.winner||'').replace('ชนะ: ',''),''),date:match.date||sport.date||''};})
  }));
}
...
function renderSelectedSportArena(sport){
  return `<article class="selected-arena state-${sport.status}"><div class="arena-sport-head"><div class="arena-big-icon">${sport.icon}</div><div><small>${escapeHTML(sport.venue)} · COMPLETE TOURNAMENT</small><h2>${escapeHTML(sport.name)}</h2>${sportCategoryTitle(sport)?`<div class="arena-category-title">${escapeHTML(sportCategoryTitle(sport))}</div>`:''}<p>${sportDateRange(sport)}</p></div><div class="arena-award ${sport.rewardType}"><small>รางวัลชนะเลิศ</small><b>${sport.rewardType==='trophy'?'🏆 ถ้วยรางวัล':'🥇 เหรียญรางวัล'}</b></div></div><div class="selected-result"><span>ผลงานสีเหลือง</span>${sport.yellowRank?`<b>${awardForSport(sport)[0]} ${awardForSport(sport)[1]}</b>`:'<b>อยู่ระหว่างการแข่งขัน</b>'}</div>${renderFullSportBracket(sport)}</article>`;
}
function updateSportsSelection(host,data,selected){
  ...
  host.querySelectorAll('[data-sport-group]').forEach(button=>button.classList.toggle('active',button.dataset.sportGroup===selected.name));
  const variantPanel=host.querySelector('.sport-variant-panel');
  if(variantPanel) variantPanel.innerHTML=`<div class="variant-panel-label"><small>ประเภทของ ${escapeHTML(selected.name)}</small><b>เลือกประเภทการแข่งขัน</b></div><div class="sport-variant-tabs">${selectedGroup.map(s=>`<button class="sport-variant ${s.id===selected.id?'active':''}" data-sport-id="${escapeAttribute(s.id)}"><span>${sportCategoryTitle(s)||'รายการทั่วไป'}</span><small>${s.status==='live'?'● กำลังแข่ง':s.status==='finished'?'✓ จบแล้ว':'○ ยังไม่เริ่ม'}</small></button>`).join('')}</div>`;
  const currentArena=host.querySelector('.selected-arena');
  if(currentArena){ currentArena.outerHTML=renderSelectedSportArena(selected); host.querySelector('.selected-arena')?.classList.add('olympic-revealed'); }
}
function bindSportsNavigation(host){
  if(host.dataset.sportsNavigationBound)return;
  host.dataset.sportsNavigationBound='true';
  host.addEventListener('click',event=>{
    const sportButton=event.target.closest('[data-sport-id]');
    if(sportButton){event.preventDefault();renderSportsPage(sportButton.dataset.sportId);return;}
    const groupButton=event.target.closest('[data-sport-group]');
    if(groupButton){event.preventDefault();const sport=normalizeSportsData().find(item=>item.name===groupButton.dataset.sportGroup);if(sport)renderSportsPage(sport.id);}
  });
}
function renderSportsPage(selectedSportId){
  ...
  bindSportsNavigation(host);
  if(host.dataset.sportsReady==='true'&&host._sportsSignature===dataSignature&&host.querySelector('.sport-explorer')){updateSportsSelection(host,data,selected);return;}
  ...
  ${renderSelectedSportArena(selected)}</section>
  ...
  host.querySelectorAll('[data-page]').forEach(b=>b.addEventListener('click',()=>showPage(b.dataset.page)));
  host.dataset.sportsReady='true';
  host._sportsSignature=dataSignature;
  initializeOlympicMotion();
}
function renderFullSportBracket(sport){ const matches=[...sport.matches].sort((a,b)=>String(a.date).localeCompare(String(b.date))); return `<div class="full-bracket"><div class="full-bracket-title"><div><small>FULL TOURNAMENT BRACKET</small><h3>ผังการแข่งขันทั้งหมด</h3></div><span>${matches.length} คู่ · ${new Set(matches.map(m=>m.date).filter(Boolean)).size} วันแข่งขัน</span></div><div class="bracket-flow">${matches.length?matches.map((m,i)=>`<div class="bracket-node ${m.winner?'decided':''} ${m.matchType==='bye'?'is-bye':''}"><div class="node-meta"><b>${escapeHTML(m.round)}</b><span>${m.date?thaiSportDate(m.date):'ยังไม่กำหนดวัน'}</span></div>${m.matchType==='bye'?`<div class="bye-result"><small>BYE · ไม่ต้องแข่งขัน</small><strong>${escapeHTML(m.byeTeam||m.a)}</strong><span>ผ่านเข้าสู่รอบถัดไป →</span></div>`:`<div class="node-team ${m.a==='สีเหลือง'?'our-team':''} ${m.winner===m.a?'winner':''}"><span>${escapeHTML(m.a)}</span><b>${escapeHTML(m.scoreA)}</b>${m.winner===m.a?'<i>WIN</i>':''}</div><div class="node-vs">VS</div><div class="node-team ${m.b==='สีเหลือง'?'our-team':''} ${m.winner===m.b?'winner':''}"><span>${escapeHTML(m.b)}</span><b>${escapeHTML(m.scoreB)}</b>${m.winner===m.b?'<i>WIN</i>':''}</div>`}${i<matches.length-1?'<div class="flow-arrow">→</div>':''}</div>`).join(''):'<div class="empty-bracket">ยังไม่มีคู่แข่งขันในกีฬานี้</div>'}</div></div>`; }
function legacyV3RenderSportsAdmin(host){
  ...
  const bind=()=>document.querySelectorAll('.sport-editor-v3').forEach(section=>{const si=+section.dataset.si;section.querySelectorAll('[data-field]').forEach(el=>el.addEventListener('input',()=>{data[si][el.dataset.field]=el.type==='number'?+el.value:el.value;dirty();}));section.querySelectorAll('[data-match]').forEach(el=>el.addEventListener('input',()=>{data[si].matches[+el.dataset.match][el.dataset.mf]=el.value;dirty();}));section.querySelector('.add-match').addEventListener('click',()=>{data[si].matches.push({round:'รอบแรก',date:'',a:'สีเหลือง',b:'สีแสด',scoreA:'-',scoreB:'-',winner:''});draw();dirty();});section.querySelectorAll('[data-remove-match]').forEach(b=>b.addEventListener('click',()=>{data[si].matches.splice(+b.dataset.removeMatch,1);draw();dirty();}));section.querySelector('[data-delete]').addEventListener('click',()=>{if(confirm(`ลบกีฬา ${data[si].name} ใช่หรือไม่?`)){data.splice(si,1);draw();dirty();}});});
...
/* Token League -------------------------------------------------------------- */
const TOKEN_KEY='phanuang-token-league-v1';
const TOKEN_PHASES=[{id:'song-1',name:'ซ้อมเพลง 1'},{id:'song-2',name:'ซ้อมเพลง 2'},{id:'song-3',name:'ซ้อมเพลง 3'},{id:'song-4',name:'ซ้อมเพลง 4'},{id:'recreation',name:'สันทนาการ'}];
function tokenWeekId(date=new Date()){const d=new Date(date);d.setHours(0,0,0,0);const day=(d.getDay()+6)%7;d.setDate(d.getDate()-day);return d.toISOString().slice(0,10)}
function getTokenState(){const today=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10),defaults={week:tokenWeekId(),cycleId:today,cycleDate:today,centralStart:0,cycleName:`รอบวันที่ ${today}`,resetAt:0,activePhase:'song-1',completedPhases:[],status:'idle',duration:15,scheduledAt:0,startedAt:0,endsAt:0,controller:null,special:false,specialValue:2,bonusRound:null,awards:[]};try{const saved=JSON.parse(localStorage.getItem(TOKEN_KEY)||'{}');if(!saved.cycleId)saved.cycleId=saved.cycleDate||today;if(!saved.cycleDate)saved.cycleDate=saved.cycleId;if(!Array.isArray(saved.completedPhases)){const index=TOKEN_PHASES.findIndex(p=>p.id===saved.activePhase);saved.completedPhases=index>0?TOKEN_PHASES.slice(0,index).map(p=>p.id):[]}saved.awards=(saved.awards||[]).map(a=>({...a,cycleId:a.cycleId||saved.cycleId}));return {...defaults,...saved}}catch(_){return defaults}}
function tokenRanking(){const state=getTokenState(),assignments=getStandConfig().assignments||{},totals={},resets=state.scoreResets||{},legacyAllResets=Object.values(state.leagueResets||{}).map(Number).filter(Number.isFinite),allReset=Math.max(Number(state.leagueResetAt||0),0,...legacyAllResets);Object.entries(assignments).forEach(([seat,student])=>totals[seat]={seat,student,score:0,firstAward:Number.POSITIVE_INFINITY});state.awards.filter(a=>{const legacySeatResets=Object.entries(resets).filter(([key])=>key.endsWith(`:${a.seat}`)).map(([,time])=>Number(time)||0),seatReset=Math.max(Number(resets[a.seat]||0),0,...legacySeatResets);return Number(a.time)>Math.max(allReset,seatReset)}).forEach(a=>{if(!totals[a.seat])return;totals[a.seat].score+=Number(a.value)||1;totals[a.seat].firstAward=Math.min(totals[a.seat].firstAward,Number(a.time)||Date.now())});return Object.values(totals).sort((a,b)=>b.score-a.score||a.firstAward-b.firstAward||a.seat.localeCompare(b.seat,undefined,{numeric:true})).slice(0,150)}
function renderTokenLeague(){const host=$('#tokenLeagueContent');if(!host)return;const state=getTokenState(),ranking=tokenRanking(),top=ranking.slice(0,3);const champ=(row,index)=>`<article class="token-champ"><span class="rank">${index+1}</span><small>ที่นั่ง ${escapeHTML(row?.seat||'—')}</small><b>${escapeHTML(row?.student?.nickname&&row.student.nickname!=='—'?row.student.nickname:row?.student?.name||'รอผู้ท้าชิง')}</b><span>${row?`ม.${escapeHTML(row.student.room||'—')}`:'ยังไม่มีคะแนน'}</span><strong>${row?.score||0}</strong><small>โทเค็น</small></article>`;host.innerHTML=`<div class="token-world"><header class="token-hero"><div><small>PHUNRUEANG · GRANDSTAND QUEST</small><h1>ลีกออฟ<br>โทเค็น</h1><p>อันดับตามข้อมูลผังแสตนด์ สูงสุด 150 คน · คะแนนสะสมข้ามวันจนกว่าแอดมินจะรีเซ็ต</p></div><div class="token-week"><small>คะแนนสะสมรายสัปดาห์</small><b>รีเซ็ตโดยแอดมินเท่านั้น</b></div></header><main class="token-board"><section class="token-podium">${[0,1,2].map(i=>champ(top[i],i)).join('')}</section><section class="token-list"><div class="token-list-head"><span>อันดับ</span><span>นักสู้ / ที่นั่ง</span><span>โทเค็น</span></div>${ranking.slice(3).map((row,index)=>`<article class="token-row"><i>#${index+4}</i><div class="token-person"><b>${escapeHTML(row.student.nickname&&row.student.nickname!=='—'?row.student.nickname:row.student.name)}</b><small>ที่นั่ง ${escapeHTML(row.seat)} · ม.${escapeHTML(row.student.room||'—')}</small></div><div class="token-score"><b>${row.score}</b><small>โทเค็น</small></div></article>`).join('')}</section></main></div>`}
async function saveTokenConfig(next){const value=JSON.stringify(next);localStorage.setItem(TOKEN_KEY,value);if(firebaseReady&&firebaseAuthUser)await saveFirebaseSharedValue(TOKEN_KEY,value)}
async function updateTokenStateTransaction(change){
  if(!firebaseReady||!firebaseSyncRoot)throw Object.assign(new Error('Firebase is not ready'),{code:'firebase/not-ready'});
  if(!firebaseAuthUser)throw Object.assign(new Error('Admin authentication is required'),{code:'auth/admin-required'});
  const result=await firebaseSyncRoot.child(firebaseKey(TOKEN_KEY)).transaction(cloud=>{let current;try{current=JSON.parse(cloud?.value||'{}')}catch(_){current={}}const next=change(current);if(!next)return;return {value:JSON.stringify(next),updatedAt:firebase.database.ServerValue.TIMESTAMP}},undefined,false);
  if(!result.committed)throw new Error('บันทึกข้อมูลไม่สำเร็จ');
  const serialized=result.snapshot.val()?.value||'{}';firebaseManualWriteKeys.add(TOKEN_KEY);try{localStorage.setItem(TOKEN_KEY,serialized)}finally{firebaseManualWriteKeys.delete(TOKEN_KEY)}return JSON.parse(serialized);
}
async function awardBonusToken(seat){
  const student=getStandConfig().assignments?.[seat],staff=JSON.parse(localStorage.getItem('phanuang-admin')||'{}');if(!student)return {ok:false,message:'ที่นั่งนี้ยังไม่มีน้องประจำแสตนด์'};
  let duplicate=false,closed=false,value=1,title='รอบพิเศษ';
  try{await updateTokenStateTransaction(current=>{const bonus=current.bonusRound;if(!bonus||bonus.status!=='open'){closed=true;return}const phase=`bonus:${bonus.id}`;if((current.awards||[]).some(a=>a.phase===phase&&a.seat===seat)){duplicate=true;return}value=Math.max(1,Math.min(20,Number(bonus.value)||1));title=bonus.title||title;current.awards=current.awards||[];current.awards.push({cycleId:current.cycleId,phase,bonusTitle:title,seat,value,time:Date.now(),staff:staff.username||staff.name||'admin'});return current})}catch(error){if(closed||duplicate)return {ok:false,message:closed?'รอบพิเศษปิดอยู่ กรุณาให้แอดมินกลางตรวจสอบ':`${seat} ได้รับโทเค็นรอบพิเศษนี้แล้ว`};throw error}
  return {ok:true,value,student,title};
}
async function mutateBonusRound(action,payload,staffId){
  return updateTokenStateTransaction(current=>{if(current.controller?.id!==staffId)throw Object.assign(new Error('เฉพาะแอดมินกลางเท่านั้นที่จัดการรอบพิเศษได้'),{code:'auth/controller-required'});const now=Date.now();if(action==='create'){current.bonusRound={id:`bonus-${now}`,title:payload.title,description:payload.description||'',value:payload.value,status:'draft',createdAt:now,startedAt:0,endedAt:0,createdBy:staffId}}else if(action==='open'){if(!current.bonusRound||current.bonusRound.status!=='draft')return;Object.assign(current.bonusRound,{title:payload.title,description:payload.description||'',value:payload.value,status:'open',startedAt:now,endedAt:0})}else if(action==='close'){if(!current.bonusRound||current.bonusRound.status!=='open')return;Object.assign(current.bonusRound,{status:'closed',endedAt:now})}else if(action==='new'){current.bonusRound=null}return current});
}
async function awardToken(seat){const state=getTokenState(),phase=state.activePhase,student=getStandConfig().assignments?.[seat],now=Date.now();if(state.status!=='running'||now<Number(state.startedAt||0)||now>=Number(state.endsAt||0))return {ok:false,message:'ช่วงนี้ยังไม่เปิดหรือหมดเวลาแล้ว กรุณาให้แอดมินกลางตรวจสอบเวลา'};if(!student)return {ok:false,message:'ที่นั่งนี้ยังไม่มีน้องประจำแสตนด์'};if(state.awards.some(a=>a.cycleId===state.cycleId&&a.phase===phase&&a.seat===seat))return {ok:false,message:`${seat} ได้โทเค็นในช่วงนี้แล้ว`};const staff=JSON.parse(localStorage.getItem('phanuang-admin')||'{}'),award={cycleId:state.cycleId,phase,seat,value:phase==='recreation'&&state.special?Math.max(1,Math.min(5,Number(state.specialValue)||1)):1,time:Date.now(),staff:staff.username||staff.name||'admin'};
  if(firebaseReady&&firebaseAuthUser&&firebaseSyncRoot){let duplicate=false,closed=false;const result=await firebaseSyncRoot.child(firebaseKey(TOKEN_KEY)).transaction(cloud=>{let current;try{current={activePhase:'song-1',status:'idle',startedAt:0,endsAt:0,special:false,specialValue:2,awards:[],...JSON.parse(cloud?.value||'{}')}}catch(_){current={activePhase:phase,status:'idle',startedAt:0,endsAt:0,special:false,specialValue:2,awards:[]}}const currentNow=Date.now();if(current.status!=='running'||currentNow<Number(current.startedAt||0)||currentNow>=Number(current.endsAt||0)){closed=true;return}if(current.awards.some(a=>(a.cycleId||current.cycleId)===current.cycleId&&a.phase===current.activePhase&&a.seat===seat)){duplicate=true;return}award.cycleId=current.cycleId;award.phase=current.activePhase;award.value=current.activePhase==='recreation'&&current.special?Math.max(1,Math.min(5,Number(current.specialValue)||1)):1;current.awards.push(award);return {value:JSON.stringify(current),updatedAt:firebase.database.ServerValue.TIMESTAMP}},undefined,false);if(!result.committed)return {ok:false,message:closed?'ช่วงนี้ปิดหรือยังไม่ถึงเวลา':duplicate?`${seat} ได้โทเค็นในช่วงนี้แล้ว`:'บันทึกไม่สำเร็จ'};localStorage.setItem(TOKEN_KEY,result.snapshot.val().value)}else{state.awards.push(award);localStorage.setItem(TOKEN_KEY,JSON.stringify(state))}return {ok:true,value:award.value,student}}
function renderTokenAdmin(host){const draw=()=>{const state=getTokenState(),phase=state.activePhase,assignments=getStandConfig().assignments||{},week=tokenWeekId(),given=new Set(state.awards.filter(a=>a.week===week&&a.phase===phase).map(a=>a.seat)),staff=JSON.parse(localStorage.getItem('phanuang-admin')||'{}'),central=staff.role==='teacher',activeName=TOKEN_PHASES.find(p=>p.id===phase)?.name||'—',total=Object.keys(assignments).length;host.innerHTML=`<div class="admin-panel token-admin token-admin-v2">
  <header class="token-command-head"><div><span class="token-step-label">TOKEN CONTROL</span><h3>แจกโทเค็นบนแสตนด์</h3><p>ทำตาม 3 ขั้นตอนด้านล่าง ระบบจะกันการแจกซ้ำให้อัตโนมัติ</p></div><div class="token-progress"><strong>${given.size}</strong><span>จาก ${total} คน</span><small>แจกแล้วในช่วงนี้</small></div></header>
  <section class="token-flow-step"><header><span>1</span><div><b>เลือกช่วงกิจกรรม</b><small>${central?'แตะเพื่อเปิดช่วงที่ต้องการ':'แอดมินกลางเป็นผู้กำหนดช่วง'}</small></div><em class="token-live-pill">● กำลังเปิด ${escapeHTML(activeName)}</em></header><nav class="token-session">${TOKEN_PHASES.map((p,index)=>`<button type="button" data-token-phase="${p.id}" class="${phase===p.id?'active':''}" ${central?'':'disabled'}><i>${p.id==='recreation'?'🎉':'♫'}</i><span><small>ช่วง ${index+1}</small><b>${p.name}</b></span>${phase===p.id?'<em>กำลังใช้</em>':''}</button>`).join('')}</nav></section>
  <section class="token-flow-step token-special-card ${phase==='recreation'?'available':'locked'}"><header><span>2</span><div><b>กำหนดจำนวนโทเค็น</b><small>${phase==='recreation'?'ช่วงสันทนาการสามารถเปิดแต้มพิเศษได้':'ช่วงซ้อมเพลงแจกครั้งละ 1 โทเค็นเท่านั้น'}</small></div><strong class="token-value-preview">+${phase==='recreation'&&state.special?state.specialValue:1} ★</strong></header><div class="token-special-controls"><label class="token-switch"><input type="checkbox" id="tokenSpecial" ${state.special?'checked':''} ${central&&phase==='recreation'?'':'disabled'}><i></i><span><b>โทเค็นพิเศษ</b><small>เปิดเพื่อแจกมากกว่า 1 แต้ม</small></span></label><label class="token-amount"><span>จำนวนต่อคน</span><select id="tokenValue" ${central&&phase==='recreation'&&state.special?'':'disabled'}>${[1,2,3,4,5].map(n=>`<option value="${n}" ${Number(state.specialValue)===n?'selected':''}>${n} โทเค็น</option>`).join('')}</select></label><p>${central?'คุณมีสิทธิ์แอดมินกลาง':'ดูได้อย่างเดียว · ควบคุมโดยแอดมินกลาง'}</p></div></section>
  <section class="token-flow-step token-pick-step"><header><span>3</span><div><b>ค้นหาแล้วแตะเพื่อแจก</b><small>การ์ดสีเขียวคือคนที่ได้รับในช่วงนี้แล้ว</small></div><em>${total-given.size} คนยังไม่ได้รับ</em></header><label class="token-search-wrap"><i>⌕</i><input class="token-seat-search" id="tokenSeatSearch" placeholder="ค้นหา เช่น J15, ชื่อเล่น หรือห้อง 1/3"><kbd>ค้นหา</kbd></label><div class="token-seat-grid" id="tokenSeatGrid">${Object.entries(assignments).map(([seat,s])=>`<button type="button" class="token-seat ${given.has(seat)?'given':''}" data-token-seat="${escapeAttribute(seat)}" ${given.has(seat)?'disabled':''}><span class="token-seat-code">${escapeHTML(seat)}</span><span class="token-seat-person"><b>${escapeHTML(s.nickname&&s.nickname!=='—'?s.nickname:s.name)}</b><small>${escapeHTML(s.name||'')}</small></span><span class="token-seat-room">ม.${escapeHTML(s.room||'—')} · เลขที่ ${escapeHTML(s.number||'—')}</span><em>${given.has(seat)?'✓ ได้รับแล้ว':'แตะเพื่อให้ +'+(phase==='recreation'&&state.special?state.specialValue:1)+' ★'}</em></button>`).join('')||'<div class="token-no-seats">ยังไม่มีข้อมูลน้องในผังที่นั่งแสตนด์</div>'}</div></section>
  <div class="token-mobile-bar"><div><small>กำลังแจก</small><b>${escapeHTML(activeName)} · +${phase==='recreation'&&state.special?state.specialValue:1} ★</b></div><span>${given.size}/${total}</span></div></div>`;
    host.querySelectorAll('[data-token-phase]').forEach(button=>button.addEventListener('click',async()=>{state.activePhase=button.dataset.tokenPhase;if(state.activePhase!=='recreation')state.special=false;await saveTokenConfig(state);draw()}));
    $('#tokenSpecial')?.addEventListener('change',async event=>{state.special=event.target.checked;await saveTokenConfig(state);draw()});$('#tokenValue')?.addEventListener('change',async event=>{state.specialValue=Number(event.target.value);await saveTokenConfig(state)});
    $('#tokenSeatSearch')?.addEventListener('input',event=>{const q=event.target.value.trim().toLowerCase();host.querySelectorAll('.token-seat').forEach(button=>button.hidden=!button.textContent.toLowerCase().includes(q))});host.querySelectorAll('[data-token-seat]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;const result=await awardToken(button.dataset.tokenSeat);if(!result.ok){button.disabled=false;window.alert(result.message);return}const toast=document.createElement('div');toast.className='token-toast';toast.textContent=`★ ${result.student.nickname&&result.student.nickname!=='—'?result.student.nickname:result.student.name} +${result.value} โทเค็น`;document.body.appendChild(toast);setTimeout(()=>toast.remove(),1800);draw()}))};draw()}

let tokenCountdownTimer=0;
var scheduleValue='';
function tokenConfirm({eyebrow='TOKEN CONTROL',title,message,confirmText='ยืนยัน',danger=false}){return new Promise(resolve=>{document.querySelector('.token-confirm-dialog')?.remove();const dialog=document.createElement('dialog');dialog.className='token-confirm-dialog';dialog.innerHTML=`<div class="token-confirm-icon">${danger?'!':'◷'}</div><small>${escapeHTML(eyebrow)}</small><h3>${escapeHTML(title)}</h3><p>${escapeHTML(message)}</p><div><button type="button" data-token-cancel>ยกเลิก</button><button type="button" class="${danger?'danger':''}" data-token-confirm>${escapeHTML(confirmText)}</button></div>`;document.body.appendChild(dialog);const done=value=>{dialog.close();dialog.remove();resolve(value)};dialog.querySelector('[data-token-cancel]').onclick=()=>done(false);dialog.querySelector('[data-token-confirm]').onclick=()=>done(true);dialog.addEventListener('cancel',e=>{e.preventDefault();done(false)});dialog.addEventListener('click',e=>{if(e.target===dialog)done(false)});dialog.showModal()})}
async function activateTokenDate(event){event?.stopImmediatePropagation();const state=getTokenState(),input=$('#tokenCentralStart'),start=new Date(input?.value).getTime();if(!Number.isFinite(start)){alert('กรุณากำหนดวันและเวลาส่วนกลาง');return}const date=new Date(start-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10),isNew=date!==state.cycleId;if(isNew&&!await tokenConfirm({title:`เปิดรอบวันที่ ${date}`,message:'เปิดรอบกิจกรรมวันใหม่และปลดล็อกทั้ง 5 ช่วง คะแนนสะสมบนลีกและประวัติเดิมจะไม่ถูกรีเซ็ต',confirmText:'เปิดรอบวันใหม่'}))return;Object.assign(state,{cycleId:date,cycleDate:date,centralStart:start,cycleName:`รอบวันที่ ${date}`,status:'idle',scheduledAt:0,startedAt:0,endsAt:0});if(isNew)Object.assign(state,{completedPhases:[],activePhase:'song-1',special:false});await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
async function resetTokenLeagueDisplay(event){event?.stopImmediatePropagation();const state=getTokenState();if(!await tokenConfirm({eyebrow:'LEAGUE DISPLAY RESET',title:'รีเซ็ตคะแนนสะสมทั้งหมด?',message:'คะแนนสะสมทุกวันที่แสดงบนหน้า “ลีกออฟโทเค็น” จะกลับเป็น 0 แต่ประวัติการให้โทเค็นและรอบกิจกรรมจะไม่ถูกลบ',confirmText:'รีเซ็ตคะแนนลีก',danger:true}))return;state.leagueResetAt=Date.now();await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
async function startTokenPhase(event){event?.stopImmediatePropagation();const state=getTokenState();if((state.completedPhases||[]).includes(state.activePhase))return;const duration=Math.max(1,Math.min(999,Number($('#tokenDuration')?.value)||15)),now=Date.now(),index=TOKEN_PHASES.findIndex(p=>p.id===state.activePhase),start=index===0&&Number(state.centralStart)>now?Number(state.centralStart):now;Object.assign(state,{duration,status:'running',scheduledAt:start,startedAt:start,endsAt:start+duration*60000});await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
document.addEventListener('click',async event=>{if(event.target?.id!=='tokenResetPerson')return;event.preventDefault();event.stopImmediatePropagation();const seat=$('#tokenResetSeat')?.value;if(!seat)return;const state=getTokenState(),row=tokenRanking().find(item=>item.seat===seat);if(!row||!confirm(`รีเซ็ตคะแนนสะสม ${row.student.nickname||row.student.name} จาก ${row.score} เป็น 0 ใช่หรือไม่?\nคะแนนจากทุกวันก่อนเวลานี้จะไม่แสดง แต่ประวัติเดิมจะยังคงอยู่`))return;state.scoreResets={...(state.scoreResets||{}),[seat]:Date.now()};await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))},{capture:true});
async function finishTokenPhase(event){event?.stopImmediatePropagation();const state=getTokenState(),index=TOKEN_PHASES.findIndex(p=>p.id===state.activePhase),next=TOKEN_PHASES[Math.min(index+1,TOKEN_PHASES.length-1)]?.id||state.activePhase;state.completedPhases=[...new Set([...(state.completedPhases||[]),state.activePhase])];state.status='idle';state.activePhase=next;state.scheduledAt=0;state.startedAt=0;state.endsAt=0;if(next!=='recreation')state.special=false;await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
async function resetAllTokenProgress(event){event?.stopImmediatePropagation();if(!confirm('รีเซ็ตคะแนนทุกคนและปลดล็อกทุกช่วงเพื่อเริ่มรอบใหม่ใช่หรือไม่?'))return;const state=getTokenState();Object.assign(state,{awards:[],completedPhases:[],resetAt:Date.now(),status:'idle',activePhase:'song-1',scheduledAt:0,startedAt:0,endsAt:0,special:false});await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
function renderTokenAdminV3(host){clearInterval(tokenCountdownTimer);const draw=()=>{clearInterval(tokenCountdownTimer);const state=getTokenState(),completed=new Set(state.completedPhases||[]),staff=JSON.parse(localStorage.getItem('phanuang-admin')||'{}'),staffId=staff.username||staff.name||'',isController=state.controller?.id===staffId,phase=state.activePhase,phaseIndex=TOKEN_PHASES.findIndex(p=>p.id===phase),activeName=TOKEN_PHASES[phaseIndex]?.name||'—',assignments=getStandConfig().assignments||{},cycleAwards=state.awards.filter(a=>a.cycleId===state.cycleId),given=new Set(cycleAwards.filter(a=>a.phase===phase).map(a=>a.seat)),running=state.status==='running',waiting=running&&Date.now()<Number(state.startedAt||0),expired=running&&Date.now()>=Number(state.endsAt||0),open=running&&!waiting&&!expired,total=Object.keys(assignments).length,tokenValue=phase==='recreation'&&state.special?state.specialValue:1,centralValue=new Date(Number(state.centralStart||Date.now())-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16),ranking=tokenRanking().filter(row=>row.score>0),history=[...state.awards].sort((a,b)=>Number(b.time)-Number(a.time)),bonus=state.bonusRound||null,bonusPhase=bonus?`bonus:${bonus.id}`:'',bonusGiven=new Set(state.awards.filter(a=>a.phase===bonusPhase).map(a=>a.seat)),bonusOpen=bonus?.status==='open';host.innerHTML=`<div class="admin-panel token-admin-v3"><header class="tv3-hero"><div><small>LIVE TOKEN OPERATIONS</small><h3>ศูนย์ควบคุมโทเค็น</h3><p>รอบปัจจุบันอิงวันที่ ${escapeHTML(state.cycleDate)} · รอบวันเก่าจะเก็บในประวัติ</p></div><div class="tv3-clock ${open?'live':expired?'expired':waiting?'waiting':'idle'}"><span>${open?'● กำลังแจก':expired?'หมดเวลา':waiting?'รอเวลาเปิด':'ยังไม่เปิดช่วง'}</span><b id="tokenCountdown">00:00</b></div></header>
<section class="tv3-controller"><div><i>⌘</i><span><small>แอดมินกลางรอบนี้</small><b>${escapeHTML(state.controller?.name||'ยังไม่มีผู้ควบคุม')}</b><em>${state.controller?'ควบคุมช่วงและเวลาอยู่':'สตาฟคนใดก็ได้กดรับหน้าที่'}</em></span></div>${!state.controller?'<button id="tokenClaim">รับหน้าที่แอดมินกลาง</button>':isController?`<button id="tokenRelease" ${running?'disabled':''}>ส่งต่อหน้าที่</button>`:'<strong>คุณเป็นสตาฟผู้แจก</strong>'}</section>
<section class="tv3-bonus ${bonus?.status||'empty'}"><header><div class="tv3-bonus-icon">✦</div><div><small>SPECIAL MISSION · แยกจาก 5 ช่วงหลัก</small><h4>รอบพิเศษ</h4><p>${bonus?escapeHTML(bonus.title):'สำหรับส่งงาน ภารกิจเพิ่มเติม หรือเก็บตกรอบซ้อม'}</p></div><span class="tv3-bonus-status">${!bonus?'ยังไม่ได้สร้าง':bonus.status==='draft'?'แบบร่าง':bonus.status==='open'?'● เปิดรับอยู่':'ปิดรอบแล้ว'}</span></header>${!bonus?`<div class="tv3-bonus-empty"><div><b>สร้างภารกิจใหม่</b><span>หนึ่งคนรับได้หนึ่งครั้งต่อรอบ ระบบจะเก็บคะแนนแยกจาก 5 ช่วงเดิม</span></div>${isController?`<div class="tv3-bonus-form"><label><span>ชื่อภารกิจ</span><input id="bonusTitle" maxlength="80" placeholder="เช่น ส่งสมุดเพลงเชียร์"></label><label><span>รายละเอียดสั้น ๆ</span><input id="bonusDescription" maxlength="160" placeholder="เช่น ส่งภายในวันศุกร์ที่โต๊ะสตาฟ"></label><label><span>โทเค็นต่อคน</span><input id="bonusValue" type="number" min="1" max="20" value="2"></label><button id="bonusCreate">＋ สร้างแบบร่าง</button></div>`:'<strong>รอแอดมินกลางสร้างรอบพิเศษ</strong>'}</div>`:`<div class="tv3-bonus-meta"><label><span>ชื่อภารกิจ</span><input id="bonusTitle" value="${escapeAttribute(bonus.title||'')}" ${isController&&bonus.status==='draft'?'':'disabled'}></label><label><span>รายละเอียด</span><input id="bonusDescription" value="${escapeAttribute(bonus.description||'')}" ${isController&&bonus.status==='draft'?'':'disabled'}></label><label><span>โทเค็นต่อคน</span><input id="bonusValue" type="number" min="1" max="20" value="${Math.max(1,Math.min(20,Number(bonus.value)||1))}" ${isController&&bonus.status==='draft'?'':'disabled'}></label><div><small>รับแล้ว</small><b>${bonusGiven.size}/${total} คน</b></div>${isController&&bonus.status==='draft'?'<button id="bonusOpen">▶ เปิดรับภารกิจ</button>':isController&&bonus.status==='open'?'<button class="close" id="bonusClose">■ ปิดรอบพิเศษ</button>':isController&&bonus.status==='closed'?'<button id="bonusNew">＋ สร้างรอบใหม่</button>':'<em>ควบคุมโดยแอดมินกลาง</em>'}</div>${bonusOpen?`<div class="tv3-bonus-award"><label class="token-search-wrap"><i>⌕</i><input id="bonusSeatSearch" placeholder="ค้นหาที่นั่ง ชื่อเล่น หรือห้อง"><kbd>ค้นหา</kbd></label><div class="token-seat-grid">${Object.entries(assignments).map(([seat,s])=>`<button class="token-seat bonus-seat ${bonusGiven.has(seat)?'given':''}" data-bonus-seat="${escapeAttribute(seat)}" ${bonusGiven.has(seat)?'disabled':''}><span class="token-seat-code">${escapeHTML(seat)}</span><span class="token-seat-person"><b>${escapeHTML(s.nickname&&s.nickname!=='—'?s.nickname:s.name)}</b><small>${escapeHTML(s.name||'')}</small></span><span class="token-seat-room">ม.${escapeHTML(s.room||'—')} · เลขที่ ${escapeHTML(s.number||'—')}</span><em>${bonusGiven.has(seat)?'✓ ส่งแล้ว':`รับงาน +${Math.max(1,Math.min(20,Number(bonus.value)||1))} ★`}</em></button>`).join('')}</div></div>`:bonus.status==='closed'?`<div class="tv3-bonus-closed"><b>สรุปรอบ “${escapeHTML(bonus.title)}”</b><span>แจกสำเร็จ ${bonusGiven.size} คน · คนละ ${Math.max(1,Math.min(20,Number(bonus.value)||1))} โทเค็น</span></div>`:''}`}</section>
<section class="tv3-central"><header><div><small>CENTRAL TOKEN DATE & TIME</small><b>วัน–เวลาส่วนกลางของการแจกโทเค็น</b><span>การเปิดวันที่ใหม่จะเริ่ม 5 ช่วงกิจกรรมใหม่ โดยคะแนนลีกยังสะสมต่อจากวันเดิม</span></div><em>รอบกิจกรรม ${escapeHTML(state.cycleDate)}</em></header><div><label><span>วันที่และเวลาเริ่มกิจกรรม</span><input id="tokenCentralStart" type="datetime-local" value="${centralValue}" ${isController&&!running?'':'disabled'}></label><button id="tokenOpenDate" onclick="activateTokenDate(event)" ${isController&&!running?'':'disabled'}>เปิดรอบตามวันที่นี้ →</button></div></section>
<section class="tv3-card"><header><span>01</span><div><b>เลือกช่วงและระยะเวลา</b><small>${isController?'วัน–เวลาใช้ค่าจากส่วนกลางด้านบน':'รอแอดมินกลางเป็นผู้กำหนด'}</small></div></header><nav class="token-session">${TOKEN_PHASES.map((p,i)=>`<button data-token-phase="${p.id}" class="${p.id===phase?'active':''} ${completed.has(p.id)||i<phaseIndex?'completed':''}" ${isController&&!running&&!completed.has(p.id)&&i>=phaseIndex?'':'disabled'}><i>${completed.has(p.id)||i<phaseIndex?'✓':p.id==='recreation'?'🎉':'♫'}</i><span><small>ช่วง ${i+1}</small><b>${p.name}</b></span>${completed.has(p.id)||i<phaseIndex?'<em>จบแล้ว</em>':''}</button>`).join('')}</nav><div class="tv3-timer tv3-schedule"><label class="schedule-date"><span>วัน–เวลาส่วนกลาง</span><input type="text" value="${new Date(Number(state.centralStart||Date.now())).toLocaleString('th-TH',{dateStyle:'short',timeStyle:'short'})}" disabled></label><label><span>ระยะเวลา</span><div><input id="tokenDuration" type="number" min="1" max="999" value="${state.duration}" ${isController&&!running?'':'disabled'}><b>นาที</b></div></label><div><small>ช่วงที่เลือก</small><b>${escapeHTML(activeName)}</b></div>${running?`<button class="finish" id="tokenFinish" onclick="finishTokenPhase(event)" ${isController?'':'disabled'}>${expired?'จบช่วงและไปช่วงถัดไป →':'■ ยกเลิก/จบช่วง'}</button>`:`<button class="start" id="tokenStart" onclick="startTokenPhase(event)" ${isController&&!completed.has(phase)?'':'disabled'}>▶ เริ่มช่วงนี้</button>`}</div></section>
<section class="tv3-card"><header><span>02</span><div><b>กำหนดจำนวนโทเค็น</b><small>${phase==='recreation'?'ช่วงสันทนาการเลือกได้ 1–5 โทเค็น':'ช่วงซ้อมเพลงแจกครั้งละ 1 โทเค็น'}</small></div><strong>+${tokenValue} ★</strong></header><div class="token-special-controls"><label class="token-switch"><input type="checkbox" id="tokenSpecial" ${state.special?'checked':''} ${isController&&!running&&phase==='recreation'?'':'disabled'}><i></i><span><b>โทเค็นพิเศษ</b><small>เปิดเฉพาะสันทนาการ</small></span></label><label class="token-amount"><span>จำนวนต่อคน</span><select id="tokenValue" ${isController&&!running&&phase==='recreation'&&state.special?'':'disabled'}>${[1,2,3,4,5].map(n=>`<option value="${n}" ${Number(state.specialValue)===n?'selected':''}>${n} โทเค็น</option>`).join('')}</select></label></div></section>
<section class="tv3-card tv3-seats ${open?'open':'closed'}"><header><span>03</span><div><b>${open?'แตะที่นั่งเพื่อแจก':'รอเริ่มช่วง'}</b><small>${open?'ทุกบัญชีสตาฟแจกได้พร้อมกัน':'ระบบจะปลดล็อกการ์ดเมื่อถึงวันและเวลาที่กำหนด'}</small></div><strong>${given.size}/${total} คน</strong></header><label class="token-search-wrap"><i>⌕</i><input class="token-seat-search" id="tokenSeatSearch" placeholder="ค้นหา J15, ชื่อเล่น หรือห้อง 1/3"><kbd>ค้นหา</kbd></label><div class="token-seat-grid">${Object.entries(assignments).map(([seat,s])=>`<button class="token-seat ${given.has(seat)?'given':''}" data-token-seat="${escapeAttribute(seat)}" ${given.has(seat)||!open?'disabled':''}><span class="token-seat-code">${escapeHTML(seat)}</span><span class="token-seat-person"><b>${escapeHTML(s.nickname&&s.nickname!=='—'?s.nickname:s.name)}</b><small>${escapeHTML(s.name||'')}</small></span><span class="token-seat-room">ม.${escapeHTML(s.room||'—')} · เลขที่ ${escapeHTML(s.number||'—')}</span><em>${given.has(seat)?'✓ ได้รับแล้ว':open?`แตะเพื่อให้ +${tokenValue} ★`:'🔒 ยังไม่เปิดช่วง'}</em></button>`).join('')}</div></section>
<section class="tv3-card tv3-reset"><header><span>04</span><div><b>จัดการคะแนนสะสมบนลีก</b><small>คะแนนรวมทุกวันจะเปลี่ยนเฉพาะเมื่อแอดมินกดรีเซ็ตในส่วนนี้</small></div></header><div class="tv3-reset-tools"><label><span>ชื่อรอบกิจกรรมวันนี้</span><input id="tokenCycleName" value="${escapeAttribute(state.cycleName)}" ${isController&&!running?'':'disabled'}></label><label><span>ผู้ที่รับรางวัลแล้ว</span><select id="tokenResetSeat" ${isController&&!running&&ranking.length?'':'disabled'}><option value="">เลือกนักเรียน…</option>${ranking.map(row=>`<option value="${escapeAttribute(row.seat)}">${escapeHTML(row.seat)} · ${escapeHTML(row.student.nickname||row.student.name)} · ${row.score} โทเค็น</option>`).join('')}</select></label><button id="tokenResetPerson" ${isController&&!running&&ranking.length?'':'disabled'}>รีเซ็ตคนนี้เป็น 0</button><button class="danger" onclick="resetTokenLeagueDisplay(event)" ${isController&&!running?'':'disabled'}>รีเซ็ตคะแนนลีกทั้งหมด</button></div></section><section class="tv3-card tv3-history"><header><span>05</span><div><b>ประวัติการให้โทเค็น</b><small>ย้อนหลังทุกวันที่ ช่วง เวลา และสตาฟผู้ให้</small></div><strong>${history.length} รายการ</strong></header><div class="tv3-history-list">${history.slice(0,500).map(a=>{const s=assignments[a.seat]||{},phaseName=TOKEN_PHASES.find(p=>p.id===a.phase)?.name||a.phase;return `<article><time>${new Date(a.time).toLocaleString('th-TH',{dateStyle:'short',timeStyle:'short'})}</time><b>${escapeHTML(a.seat)} · ${escapeHTML(s.nickname||s.name||'ไม่พบข้อมูล')}</b><span>${escapeHTML(a.cycleId||state.cycleId)} · ${escapeHTML(phaseName)}</span><strong>+${Number(a.value)||1} ★</strong><small>โดย ${escapeHTML(a.staff||'admin')}</small></article>`}).join('')||'<p>ยังไม่มีประวัติการให้โทเค็น</p>'}</div></section></div>`;
$('#tokenRelease')?.toggleAttribute('disabled',running||bonusOpen);if(bonusOpen){$('#tokenOpenDate')?.setAttribute('disabled','');host.querySelectorAll('.tv3-reset input,.tv3-reset select,.tv3-reset button').forEach(control=>control.disabled=true)}host.querySelectorAll('.tv3-history-list article').forEach((article,index)=>{const award=history[index];if(!award?.phase?.startsWith('bonus:'))return;const label=article.querySelector('span');if(label)label.textContent=`${award.cycleId||state.cycleId} · รอบพิเศษ: ${award.bonusTitle||'ภารกิจพิเศษ'}`});
const bonusPayload=()=>({title:$('#bonusTitle')?.value.trim()||'',description:$('#bonusDescription')?.value.trim()||'',value:Math.max(1,Math.min(20,Number($('#bonusValue')?.value)||1))}),runBonus=async(action)=>{const payload=bonusPayload();if(['create','open'].includes(action)&&!payload.title){alert('กรุณากรอกชื่อภารกิจก่อน');$('#bonusTitle')?.focus();return}try{await mutateBonusRound(action,payload,staffId);draw()}catch(error){alert(error?.message||'จัดการรอบพิเศษไม่สำเร็จ')}};
$('#bonusCreate')?.addEventListener('click',()=>runBonus('create'));$('#bonusOpen')?.addEventListener('click',async()=>{if(await tokenConfirm({eyebrow:'SPECIAL MISSION',title:`เปิดรับ “${bonusPayload().title}”`,message:`สตาฟทุกคนจะเริ่มแจกได้ คนละ ${bonusPayload().value} โทเค็น และหนึ่งคนรับได้ครั้งเดียว`,confirmText:'เปิดรับรอบพิเศษ'}))runBonus('open')});$('#bonusClose')?.addEventListener('click',async()=>{if(await tokenConfirm({eyebrow:'SPECIAL MISSION',title:'ปิดรอบพิเศษนี้?',message:`แจกแล้ว ${bonusGiven.size} จาก ${total} คน เมื่อปิดแล้วจะไม่สามารถแจกเพิ่มในรอบนี้ได้`,confirmText:'ปิดรอบ'}))runBonus('close')});$('#bonusNew')?.addEventListener('click',()=>runBonus('new'));$('#bonusSeatSearch')?.addEventListener('input',event=>{const q=event.target.value.trim().toLowerCase();host.querySelectorAll('[data-bonus-seat]').forEach(button=>button.hidden=!button.textContent.toLowerCase().includes(q))});host.querySelectorAll('[data-bonus-seat]').forEach(button=>button.addEventListener('click',async()=>{button.disabled=true;try{const result=await awardBonusToken(button.dataset.bonusSeat);if(!result.ok){button.disabled=false;alert(result.message);return}const toast=document.createElement('div');toast.className='token-toast bonus';toast.textContent=`✦ ${result.student.nickname||result.student.name} ส่งงานแล้ว +${result.value} โทเค็น`;document.body.appendChild(toast);setTimeout(()=>toast.remove(),1800);draw()}catch(error){button.disabled=false;alert(error?.message||'แจกโทเค็นไม่สำเร็จ')}}));
const save=async next=>{await saveTokenConfig(next);draw()};$('#tokenClaim')?.addEventListener('click',()=>save({...state,controller:{id:staffId,name:staff.name||staff.username||'สตาฟ',since:Date.now()}}));$('#tokenRelease')?.addEventListener('click',()=>save({...state,controller:null}));host.querySelectorAll('[data-token-phase]').forEach(b=>b.addEventListener('click',()=>{const next=b.dataset.tokenPhase;save({...state,activePhase:next,special:next==='recreation'?state.special:false})}));$('#tokenDuration')?.addEventListener('change',e=>{state.duration=Math.max(1,Math.min(999,Number(e.target.value)||1))});$('#tokenStart')?.addEventListener('click',()=>{const duration=Math.max(1,Math.min(999,Number($('#tokenDuration').value)||15)),scheduled=new Date($('#tokenScheduledAt').value).getTime();if(!Number.isFinite(scheduled)){alert('กรุณากำหนดวันและเวลาเริ่ม');return}save({...state,duration,scheduledAt:scheduled,status:'running',startedAt:scheduled,endsAt:scheduled+duration*60000})});$('#tokenFinish')?.addEventListener('click',()=>{const next=TOKEN_PHASES[Math.min(phaseIndex+1,TOKEN_PHASES.length-1)].id;save({...state,status:'idle',activePhase:next,scheduledAt:0,startedAt:0,endsAt:0,special:next==='recreation'?state.special:false})});$('#tokenSpecial')?.addEventListener('change',e=>save({...state,special:e.target.checked}));$('#tokenValue')?.addEventListener('change',e=>save({...state,specialValue:Number(e.target.value)}));$('#tokenCycleName')?.addEventListener('change',e=>save({...state,cycleName:e.target.value.trim()||'รอบกิจกรรมปัจจุบัน'}));$('#tokenSeatSearch')?.addEventListener('input',e=>{const q=e.target.value.trim().toLowerCase();host.querySelectorAll('.token-seat').forEach(b=>b.hidden=!b.textContent.toLowerCase().includes(q))});host.querySelectorAll('[data-token-seat]').forEach(b=>b.addEventListener('click',async()=>{b.disabled=true;const result=await awardToken(b.dataset.tokenSeat);if(!result.ok){b.disabled=false;alert(result.message);return}const toast=document.createElement('div');toast.className='token-toast';toast.textContent=`★ ${result.student.nickname||result.student.name} +${result.value} โทเค็น`;document.body.appendChild(toast);setTimeout(()=>toast.remove(),1800);draw()}));const tick=()=>{const el=$('#tokenCountdown');if(!el)return;const target=waiting?Number(state.startedAt):Number(state.endsAt),left=Math.max(0,target-Date.now()),m=Math.floor(left/60000),s=Math.floor(left%60000/1000);el.textContent=`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;if((open||waiting)&&left<=0){clearInterval(tokenCountdownTimer);setTimeout(draw,0)}};tick();if(open||waiting)tokenCountdownTimer=setInterval(tick,1000)};draw()}
...
function enhanceSportByeControls(data,dirty,draw){
  document.querySelectorAll('.sport-builder-card').forEach(card=>{
    const si=Number(card.dataset.si);
    card.querySelectorAll('.match-builder-row').forEach((row,mi)=>{
      if(row.querySelector('[data-bye-control]'))return;
      const match=data[si]?.matches[mi];if(!match)return;
      row.classList.toggle('is-bye',match.matchType==='bye');
      const label=document.createElement('label');label.className='bye-control';
      label.innerHTML=`<span>รูปแบบการแข่งขัน</span><select data-bye-control><option value="normal" ${match.matchType!=='bye'?'selected':''}>แข่งขันปกติ</option>${SPORT_COLORS.map(color=>`<option value="bye:${color}" ${match.matchType==='bye'&&match.byeTeam===color?'selected':''}>บาย · ${color} ผ่านเข้ารอบ</option>`).join('')}</select><small>${match.matchType==='bye'?'ไม่ต้องกรอกคะแนน สีที่เลือกจะผ่านเข้ารอบทันที':'เลือก “บาย” เมื่อรอบนี้ไม่ต้องแข่งขัน'}</small>`;
      row.querySelector('.match-number')?.insertAdjacentElement('afterend',label);
      label.querySelector('select').addEventListener('change',event=>{const [type,color]=event.target.value.split(':');if(type==='bye'){match.matchType='bye';match.byeTeam=color;match.a=color;match.winner=color;match.scoreA='-';match.scoreB='-';}else{match.matchType='normal';match.byeTeam='';match.winner='';}dirty();draw();document.querySelector(`.sport-builder-card[data-si="${si}"]`)?.setAttribute('open','');});
    });
  });
}
...
function renderSportsAdmin(host){
  ...
  const categoryObserver=new MutationObserver(()=>{enhanceSportCategoryControls(data,dirty);enhanceSportLevelControls(data,dirty);enhanceSportByeControls(data,dirty,draw);updateTournamentSummary(data);});
  ...
  const addMatch=si=>{data[si].matches.push({round:data[si].matches.length?'รอบถัดไป':'รอบแรก',date:'',a:'สีเหลือง',b:'สีแสด',scoreA:'-',scoreB:'-',winner:''});draw();dirty();window.setTimeout(()=>{const card=document.querySelector(`[data-si="${si}"]`);card.open=true;card.querySelector('.match-builder-row:last-child')?.scrollIntoView({behavior:'smooth',block:'center'});},0);};
