...
// เปลี่ยนรายการนี้เป็นข้อมูลพี่สตาฟจากฐานข้อมูลจริงก่อนเปิดใช้งานจริง
const STAFF_ACCOUNTS = [{ studentId: '65001', password: '0113', name: 'พี่สตาฟตัวอย่าง' }];
...
function renderAdmin() {
  ...
  if (!staff) { host.innerHTML = `<div class="admin-login-shell"><div class="admin-login-visual"><small>PHUNRUEANG STAFF</small><div class="login-sigil"><i>✦</i><b>PR</b></div><h3>เบื้องหลังทุกชัยชนะ<br>คือทีมที่พร้อมเสมอ</h3><p>พื้นที่ปฏิบัติการสำหรับทีมงานคณะสีพันเรือง</p><div class="login-status"><span></span> SECURE STAFF ACCESS</div></div><div class="admin-login"><div class="login-step">01 / AUTHENTICATION</div><h3>ยินดีต้อนรับกลับ</h3><p>ยืนยันตัวตนเพื่อเข้าสู่ศูนย์บัญชาการ</p><form id="adminLoginForm"><div class="field"><label>เลขประจำตัวนักเรียน</label><div class="admin-input"><span>◉</span><input id="adminStudentId" required inputmode="numeric" autocomplete="username" placeholder="เช่น 65001"></div></div><div class="field"><label>รหัสผ่าน</label><div class="admin-input"><span>◆</span><input id="adminPassword" required type="password" inputmode="numeric" autocomplete="current-password" placeholder="••••"></div></div><div class="error" id="adminLoginError"></div><button class="primary">เข้าสู่ COMMAND CENTER <b>→</b></button></form><small class="admin-secure-note">🔒 ข้อมูลของคุณได้รับการปกป้อง</small></div></div>`; $('#adminLoginForm').addEventListener('submit', (event) => { event.preventDefault(); const account = STAFF_ACCOUNTS.find((item) => item.studentId === $('#adminStudentId').value && item.password === $('#adminPassword').value); if (!account) { $('#adminLoginError').textContent = 'ไม่พบข้อมูลพี่สตาฟ หรือรหัสผ่านไม่ถูกต้อง'; return; } localStorage.setItem('phanuang-admin', JSON.stringify(account)); renderAdmin(); }); return; }
  host.innerHTML = `<div class="admin-workspace"><header class="admin-top"><div class="staff-avatar">${staff.name.slice(0,1)}</div><div><small>WELCOME BACK, STAFF</small><p><b>${staff.name}</b> <span>ออนไลน์</span></p></div><div class="admin-live"><i></i> SYSTEM LIVE</div><button class="mini" id="adminLogout">ออกจากระบบ ↗</button></header><nav class="admin-tabs" aria-label="เมนูผู้ดูแล"><button class="admin-tab active" data-admin-tab="attendance"><i>✓</i><span>เช็คชื่อ<small>ATTENDANCE</small></span></button><button class="admin-tab" data-admin-tab="attendance-history"><i>◴</i><span>ประวัติเช็คชื่อ<small>HISTORY</small></span></button><button class="admin-tab" data-admin-tab="sports"><i>🏆</i><span>ผลกีฬา<small>SPORTS</small></span></button><button class="admin-tab" data-admin-tab="members"><i>♙</i><span>สมาชิก<small>MEMBERS</small></span></button><button class="admin-tab" data-admin-tab="store"><i>◆</i><span>จัดการร้านค้า<small>STORE MANAGER</small></span></button><button class="admin-tab" data-admin-tab="orders"><i>▤</i><span>คำสั่งซื้อ<small>ORDERS & CSV</small></span></button><button class="admin-tab" data-admin-tab="photos"><i>◫</i><span>รูปกิจกรรม<small>GALLERY</small></span></button><button class="admin-tab" data-admin-tab="election"><i>✦</i><span>เลือกตั้ง<small>ELECTION</small></span></button></nav><div id="adminPanel"></div></div>`;
