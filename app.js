...
function renderActivityAttendance(host){const data=getActivityData(),directory=attendanceStudentDirectory();let group=localStorage.getItem('phanuang-active-group')||ACTIVITY_GROUPS[0],roster=data.rosters[group]||[],session=data.sessions.filter(s=>s.group===group)[0],today=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);
  ...
  document.querySelectorAll('[data-delete-activity]').forEach(button=>button.onclick=async event=>{event.preventDefault();event.stopPropagation();if(!confirm('ลบประวัติรอบนี้เท่านั้น? รอบล่าสุดจะไม่ถูกกระทบ'))return;button.disabled=true;try{await updateActivityData(current=>{current.sessions=current.sessions.filter(item=>item.id!==button.dataset.deleteActivity);return current});renderActivityAttendance(host);showAdminToast('ลบประวัติแล้ว','ลบเฉพาะรอบที่เลือก · รอบล่าสุดยังอยู่ครบ')}catch(error){button.disabled=false;window.alert(adminSaveErrorMessage(error))}});
