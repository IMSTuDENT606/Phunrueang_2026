...
function renderSeatMap() {
  ...
  map.innerHTML = seatCodes.map((seat) => {const s=assignments[seat];return `<span class="seat ${attendance[seat] ? 'present' : 'absent'} ${s?'assigned':''}" data-seat="${seat}" title="${s?`${escapeAttribute(s.name)} · ม.${s.room} เลขที่ ${s.number}`:'ยังไม่ได้ผูกนักเรียน'}${attendance[seat] ? ` · เช็คชื่อ ${new Date(attendance[seat].time).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})} น.` : ''}"><b>${seat}</b>${s?`<small>${escapeHTML(s.nickname==='—'?s.name.split(' ')[0]:s.nickname)}</small>`:''}</span>`}).join('');
...
const ACTIVITY_GROUPS=['บาสเกตบอล','วอลเลย์บอล','แบดมินตัน','เปตอง','เซปักตะกร้อ','กรีฑา','ฟุตบอล','กระโดดไกล','ขว้างจักร','ฟุตซอล','ดรัมเมเยอร์','เชียร์ลีดเดอร์','ขบวนพาเหรด'];
function getActivityData(){try{return JSON.parse(localStorage.getItem('phanuang-activity-attendance')||'{"rosters":{},"sessions":[]}')}catch(_){return {rosters:{},sessions:[]}}}
function openActivityStudentPicker({directory,roster,group,data,host}){
  ...
  $('#saveSelectedStudents').onclick=()=>{data.rosters[group]=directory.filter(s=>selected.has(s.id));localStorage.setItem('phanuang-activity-attendance',JSON.stringify(data));close();renderActivityAttendance(host);showAdminToast('อัปเดตรายชื่อแล้ว',`${group} จำนวน ${selected.size} คน`)};
...
function renderActivityAttendance(host){const data=getActivityData(),directory=attendanceStudentDirectory();let group=localStorage.getItem('phanuang-active-group')||ACTIVITY_GROUPS[0],roster=data.rosters[group]||[],session=data.sessions.filter(s=>s.group===group)[0],today=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);
  ...
  $('#activityGroupSelect').onchange=e=>{localStorage.setItem('phanuang-active-group',e.target.value);renderActivityAttendance(host)};
  ...
  $('#saveActivitySession').onclick=()=>{const date=$('#activityDate').value,present=[...document.querySelectorAll('[data-student]:checked')].map(x=>x.dataset.student),record={id:`activity-${Date.now()}`,group,date,present,roster:roster.map(s=>s.id),createdAt:new Date().toISOString()};data.sessions=data.sessions.filter(s=>!(s.group===group&&s.date===date));data.sessions.unshift(record);localStorage.setItem('phanuang-activity-attendance',JSON.stringify(data));renderActivityAttendance(host);showAdminToast('บันทึกผลเช็คชื่อแล้ว',`${group} มา ${present.length}/${roster.length} คน`)};
