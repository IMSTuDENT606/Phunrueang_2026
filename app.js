...
async function prepareImageForStorage(file, options = {}) {
  if (!(file instanceof Blob) || !String(file.type || '').startsWith('image/')) throw new Error('กรุณาเลือกไฟล์รูปภาพ');
...
function adminSaveErrorMessage(error) {
  ...
  if (/quota|storage/i.test(code)) return 'พื้นที่เก็บข้อมูลของเบราว์เซอร์เต็ม กรุณาลบข้อมูลเว็บไซต์เก่าหรือรูปที่ไม่ใช้แล้ว';
...
async function renderStoreAdmin(host){
  ...
  const bind=()=>{
    const fileData=(file,isImage)=>isImage?prepareImageForStorage(file,{maxSide:1400,maxBytes:350000}):new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(reader.error);reader.readAsDataURL(file);});
...
function renderAdminPanel(tab) {
  ...
  if (tab === 'photos') {
    ...
    const compressActivityImage = (file) => typeof file !== 'string' ? prepareImageForStorage(file, { maxSide: 960, maxBytes: 220000 }) : new Promise((resolve, reject) => {
    ...
    $('#saveGallery').addEventListener('click', () => { if (syncGallery()) window.alert('บันทึกการตั้งค่ารูปภาพกิจกรรมแล้ว'); });
    renderPreviews();
    return;
  }
...
async function renderCommitteeAdmin(host) {
  ...
  members = await Promise.all(members.map(async (member) => ({ ...member, image: await shrinkLegacyImage(member.image, { maxSide: 1400, maxBytes: 300000 }) })));
  ...
  const bindRows = () => { list.querySelectorAll('.member-editor-row').forEach((el) => { el.querySelector('.member-delete').addEventListener('click', () => { syncFromRows(); members.splice(Number(el.dataset.index),1); renderRows(); }); el.querySelector('input[type=file]').addEventListener('change', async (event) => { const file = event.target.files[0]; if (!file) return; const memberIndex = Number(el.dataset.index); try { const prepared = await prepareImageForStorage(file, { maxSide: 1400, maxBytes: 300000 }); syncFromRows(); if (!members[memberIndex]) return; openMemberCropStudio(prepared, (croppedImage) => { if (!members[memberIndex]) return; members[memberIndex].image = croppedImage; renderRows(); }); } catch (error) { window.alert(error.message || 'ไม่สามารถอ่านรูปภาพนี้ได้'); } }); }); };
  ...
  $('#saveCommitteeMembers').addEventListener('click', async () => { syncFromRows(); const button = $('#saveCommitteeMembers'); button.disabled = true; $('#memberSaveStatus').textContent = 'กำลังบันทึก…'; try { await setCommitteeMembers(members); localStorage.setItem(COMMITTEE_KEY, JSON.stringify(members)); renderCommitteeMembers(true); $('#memberSaveStatus').textContent = `บันทึกแล้ว ${members.length} การ์ด`; } catch (error) { $('#memberSaveStatus').textContent = 'บันทึกไม่สำเร็จ'; window.alert(adminSaveErrorMessage(error)); } finally { button.disabled = false; } });
  renderRows();
}
