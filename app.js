...
const ACTIVITY_GROUPS=['บาสเกตบอล','วอลเลย์บอล','แบดมินตัน','เปตอง','เซปักตะกร้อ','กรีฑา','ฟุตบอล','กระโดดไกล','ขว้างจักร','ฟุตซอล','ดรัมเมเยอร์','เชียร์ลีดเดอร์','ขบวนพาเหรด','สวัสดิการ/พยาบาล','คัตเอาท์'];
function normalizeActivityData(value){return {rosters:value?.rosters&&typeof value.rosters==='object'?value.rosters:{},sessions:Array.isArray(value?.sessions)?value.sessions:[]}}
...
function renderActivityAttendance(host){const data=getActivityData(),directory=attendanceStudentDirectory();let group=localStorage.getItem('phanuang-active-group')||ACTIVITY_GROUPS[0],roster=data.rosters[group]||[],session=data.sessions.filter(s=>s.group===group)[0],today=new Date(Date.now()-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10);
  ...
  document.querySelectorAll('[data-student]').forEach(x=>x.onchange=syncChecks);
  $('#markAllActivity').onclick=()=>{document.querySelectorAll('[data-student]').forEach(x=>x.checked=true);syncChecks()};
  $('#clearAllActivity').onclick=()=>{document.querySelectorAll('[data-student]').forEach(x=>x.checked=false);syncChecks()};
  $('#newActivitySession').onclick=()=>{session=null;document.querySelectorAll('[data-student]').forEach(x=>x.checked=false);$('#activityDate').value=today;syncChecks()};
...
async function activateTokenDate(event){event?.stopImmediatePropagation();const state=getTokenState(),input=$('#tokenCentralStart'),start=new Date(input?.value).getTime();if(!Number.isFinite(start)){alert('กรุณากำหนดวันและเวลาส่วนกลาง');return}const date=new Date(start-new Date().getTimezoneOffset()*60000).toISOString().slice(0,10),isNew=date!==state.cycleId;if(isNew&&!await tokenConfirm({title:`เปิดรอบวันที่ ${date}`,message:'คะแนนบนลีกจะเริ่มที่ 0 และปลดล็อกทั้ง 5 ช่วงสำหรับวันใหม่ ส่วนประวัติการให้โทเค็นเดิมจะยังเก็บไว้ครบถ้วน',confirmText:'เปิดรอบวันใหม่'}))return;Object.assign(state,{cycleId:date,cycleDate:date,centralStart:start,cycleName:`รอบวันที่ ${date}`,status:'idle',scheduledAt:0,startedAt:0,endsAt:0});if(isNew)Object.assign(state,{completedPhases:[],activePhase:'song-1',special:false});await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
async function resetTokenLeagueDisplay(event){event?.stopImmediatePropagation();const state=getTokenState();if(!await tokenConfirm({eyebrow:'LEAGUE DISPLAY RESET',title:'รีเซ็ตคะแนนบนลีกทั้งหมด?',message:`คะแนนที่แสดงบนหน้า “ลีกออฟโทเค็น” ของวันที่ ${state.cycleDate} จะกลับเป็น 0 แต่ประวัติและสถานะช่วงจะไม่ถูกลบ`,confirmText:'รีเซ็ตการแสดงผล',danger:true}))return;state.leagueResets={...(state.leagueResets||{}),[state.cycleId]:Date.now()};await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))}
...
document.addEventListener('click',async event=>{if(event.target?.id!=='tokenResetPerson')return;event.preventDefault();event.stopImmediatePropagation();const seat=$('#tokenResetSeat')?.value;if(!seat)return;const state=getTokenState(),row=tokenRanking().find(item=>item.seat===seat);if(!row||!confirm(`รีเซ็ตคะแนน ${row.student.nickname||row.student.name} จาก ${row.score} เป็น 0 ใช่หรือไม่?\nประวัติการให้เดิมจะยังคงอยู่`))return;state.scoreResets={...(state.scoreResets||{}),[`${state.cycleId}:${seat}`]:Date.now()};await saveTokenConfig(state);renderTokenAdminV3($('#adminPanel'))},{capture:true});
